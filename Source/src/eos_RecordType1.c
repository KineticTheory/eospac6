 ...
 *                                               contents of object are
 *initialized \param[in]     materialID - EOS_INTEGER     : id of material to
 *load \param[in]     th         - EOS_INTEGER     : table handle
 *
 * \return none
 *
 ***********************************************************************/
void eos_ConstructRecordType1(eos_RecordType1 *me, EOS_INTEGER th,
  ...
  me->temporary = NULL; /* used exclusively for temporary data exchange; not
                           packed/unpacked */
  ...
  me->eosData.CheckExtrap =
      eos_CheckExtrapRecordType1_using_extrapolationBounds;
...
void eos_CreateRecordType1(void *ptr, EOS_INTEGER th) {
  ...
  if ((eos_GetStandardErrorCodeFromCustomErrorCode(
           gEosDataMap.errorCodes[th]) == EOS_DATA_TYPE_NOT_FOUND) &&
      !me->eosData.forceCreate) {
    // DAP - no error handling is done here so that eos_LoadRecordType1 can
    // decide what to do in the case
  ...
  if (me->eosData.dataSize <
      2 + me->NR + me->NT + me->NR * me->NT) { /* at least 1 subtable! */
...
void eos_SetFileIndexesRecordType1(void *ptr, EOS_INTEGER th) {
  ...
  {
    /* get the file offset for reading data and get back 6 reals: NR, NT, R[0],
     * date1, date2 and version */
    ses_material_id mid = (ses_material_id)me->eosData.materialID;
    ses_table_id tid = (ses_table_id)me->eosData.tableNum;
    nreals_to_read = 6;
    ierr = eos_SesSeekToDataTable(matid, me->eosData.tableNum, &read_data,
                                  nreals_to_read, &(me->eosData.dataFileIndex),
                                  &mid, &tid, &(me->eosData.dataSize),
                                  me->eosData.userDefinedDataFile, &errMsg);
    if (errMsg)
      ierr = eos_SetCustomErrorMsg(th, ierr, "%s", errMsg);
    EOS_FREE(errMsg);
  }
  if ((eos_GetStandardErrorCodeFromCustomErrorCode(ierr) ==
       EOS_DATA_TYPE_NOT_FOUND) &&
      !me->eosData.forceCreate) {
    // DAP - no error handling is done here so that eos_LoadRecordType1 can
    // decide what to do in the case
    //       that a Sesame table is missing.
    /* deallocate read_data[], which was allocated by eos_SeekToDataTable()
     * above */
  ...
  if (nreals_to_read > 5) {
    ...
    me->tabulated_rhozero_exists =
        (read_data[count++] > 0.0) ? EOS_FALSE : EOS_TRUE;
  }
  /* read creation date */
...
#ifdef MY_DEBUG
  printf(
      "In SetFileIndexes Record Type 1 after EOS_FREE read_data returning\n");
#endif
}
...
/***********************************************************************/
/*!
 * \brief This function loads data of RecordType1 and stores the data in the
...
void eos_LoadRecordType1(void *ptr, EOS_INTEGER th) {
  eos_RecordType1 *me;
  EOS_INTEGER tableNum, i, j, count, index1, index2, index3, index4;
  EOS_INTEGER ierr = EOS_OK;
  ...
  EOS_BOOLEAN freeEnergyIsMissing, use_taylor_fit = EOS_FALSE,
                                   use_maxwell_table = EOS_FALSE;
  EOS_CHAR *errMsg = NULL;
  ...
  if (eos_GetStandardErrorCodeFromCustomErrorCode(gEosDataMap.errorCodes[th]) ==
      EOS_MATERIAL_NOT_FOUND) {
  ...
  ierr = eos_SesGetBulkData(
      me->eosData.materialID, me->eosData.userDefinedDataFile,
      me->eosData.dataFileIndex, &(me->avgAtomicNumber), &(me->avgAtomicWgt),
      &(me->refDensity), &(me->solidBulkModulus), &(me->exchangeCoefficient),
      &errMsg);
  if (errMsg)
    ierr = eos_SetCustomErrorMsg(th, ierr, "%s", errMsg);
  EOS_FREE(errMsg);
  if (ierr) {
    ...
    ierr = eos_SetCustomErrorMsg(th, ierr,
                                 "eos_RecordType1::eos_GetBulkData ERROR, bulk "
                                 "data not available for table handle, %i",
                                 th);
    return;
  }
  ...
  if (use_taylor_fit) {
    ...
    /* create and populate Taylor objects for each P, U, A and S as required for
     * available data */
    if (isOneDimDatatype) {
      for (i = 0; i < 4; i++) {
        ierr = eos_Load1DTaylor(&(me->eosData),
                                (ses_material_id)me->eosData.materialID,
                                (ses_table_id)tableNum, i + 1, &(me->TX),
                                &(me->M), &(me->Taylor_objects[i]));
        if (eos_GetStandardErrorCodeFromCustomErrorCode(ierr) ==
            EOS_NO_DATA_TABLE) {
        ...
        if (ierr)
          break; /* exit loop and handle error */
        me->eosData.numSubtablesLoaded++;
      }
    } else {
      for (i = 0; i < 4; i++) {
        ierr = eos_Load2DTaylor(
            &(me->eosData), (ses_material_id)me->eosData.materialID,
            (ses_table_id)tableNum, i + 1, &(me->TX), &(me->TY), &(me->M),
            &(me->N), &(me->Taylor_objects[i]));
        if (eos_GetStandardErrorCodeFromCustomErrorCode(ierr) ==
            EOS_NO_DATA_TABLE) {
          EOS_FREE(me->Taylor_objects[i]); /* free memory and nullify pointer */
          continue;                        /* skip remaining */
    ...
    if (ierr) {
      ((eos_ErrorHandler *)me)->HandleError(me, th, ierr);
      ierr = eos_SetCustomErrorMsg(
          th, ierr,
          "eos_LoadRecordType1 ERROR, no Taylor polynomial fit data available "
          "for table handle, %i",
          th);
      return;
    }
    ...
    for (i = 0; i < 4; i++) {
      if (me->Taylor_objects[i])
        me->_eos_EvaluateTaylor[i] = _eos_EvaluateTaylorFor_dataType_DT;
    }
    i = 0;
    if (!me->Taylor_objects[i] && me->Taylor_objects[2])
      me->_eos_EvaluateTaylor[i] = _eos_EvaluateTransformedTaylorFor_Pt_DT;
    i = 1;
    if (!me->Taylor_objects[i])
      me->_eos_EvaluateTaylor[i] = _eos_EvaluateTransformedTaylorFor_Ut_DT;
    i = 3;
    if (!me->Taylor_objects[i])
      me->_eos_EvaluateTaylor[i] = _eos_EvaluateTransformedTaylorFor_St_DT;
  ...
  /* Load the read_data array with all of the available data. If necessary,
     use the selected analytical model for ions to create 303 or 304 Sesame
     table data */
  splitOptFlag = -1;
  if (tableNum == 303 || tableNum == 304 ||
      tableNum == 305) /* model only valid to create 303, 304 or 305 data */
  ...
  if (!(optValForced && optValForced->bval)) {
    /* just load data from Sesame file, if available, or do unforced splitting
     * if requested */

    if (me->eosData.dataSize <
        2) { /* eos_CreateRecordType1 failed to load data */

      if (splitOptFlag != -1 && (tableNum == 303 || tableNum == 304)) {
      ...
      } else { /* tableNum doesn't exist in Sesame for current matID and no
                  splitting requested */
      ...
      ierr = eos_SesLoadSesameFiles(
          me->eosData.materialID, me->eosData.tableNum,
          me->eosData.dataFileIndex, &read_data,
          me->eosData.dataSize - 2); /* DATA SIZE INCLUDES DATA ALREADY READ */
      if (ierr) {
        ((eos_ErrorHandler *)me)->HandleError(me, th, ierr);
        return;
      }

      if ((tableNum >= 301 && tableNum <= 306) ||
          (tableNum >= 311 && tableNum <= 316)) {
        /* load entropy subtable if possible */

        EOS_REAL *temp_mod = NULL, *enion_mod = NULL, *anion_mod = NULL,
                 *s_ion_mod = NULL;
        EOS_BOOLEAN create_T0_data = EOS_FALSE;

        freeEnergyIsMissing = EOS_FALSE;
        if (me->eosData.dataSize <=
            (2 + me->NR + me->NT + 2 * me->NR * me->NT)) {
        ...
        if (optVal->bval) {
          ...
          me->eosData.altDataSource = (EOS_CHAR *)malloc(
              (eos_appendedSourceStrL + sesame_fnameL + 1) * sizeof(EOS_CHAR));
          me->eosData.altDataSource =
              strcpy(me->eosData.altDataSource, sesame_fname);
          me->eosData.altDataSource =
              strcat(me->eosData.altDataSource, eos_appendedSourceStr);
        }
        ...
        if (freeEnergyIsMissing && temp[0] > ZERO && optVal->bval) {
          /* cold curve data and Free energy are unavailable, so linearly
           * extrapolate U(rho,T=0) to allow entropy and free energy tables to
           * be created
           */
          create_T0_data = EOS_TRUE;
          temp_mod = (EOS_REAL *)malloc(
              (me->NT + 1) *
              sizeof(EOS_REAL)); // allocate temporary T array to include T=0
        ...
        if ((freeEnergyIsMissing && temp[0] <= ZERO) || create_T0_data) {
          ...
          // cold curve data is available so free energy and entropy tables can
          // be constructed

          // Store the total size of the data to be stored for all subtables:
          // NR, NT, R[], T[], table1[], table2[], table3[], and table4[]
          me->eosData.dataSize =
              2 + me->NR + me->NT + MAX_TABLES_RECORDTYPE1 * me->NR * me->NT;
          ...
          if (create_T0_data) {
            enion_mod =
                (EOS_REAL *)malloc(me->NR * (me->NT + 1) * sizeof(EOS_REAL));
            anion_mod =
                (EOS_REAL *)malloc(me->NR * (me->NT + 1) * sizeof(EOS_REAL));
            s_ion_mod =
                (EOS_REAL *)malloc(me->NR * (me->NT + 1) * sizeof(EOS_REAL));
          ...
          if (create_T0_data)
            ierr = eos_Entropy(me, me->NR, me->NT + 1, enion_mod, EOS_NullPtr,
                               temp, dens, s_ion_mod);
          else
            ierr = eos_Entropy(me, me->NR, me->NT, enion, EOS_NullPtr, temp,
                               dens, s_ion);
          if (eos_GetStandardErrorCodeFromCustomErrorCode(ierr) != EOS_OK) {
            /* ignore the error, just set the dataSize and numTables loaded */
            me->eosData.dataSize = 2 + me->NR + me->NT +
                                   2 * me->NR * me->NT; /* only TWO subtables */
          ...
          ierr =
              eos_Entropy(me, me->NR, me->NT, enion, anion, temp, dens, s_ion);
          if (eos_GetStandardErrorCodeFromCustomErrorCode(ierr) != EOS_OK) {
            /* ignore the error, just set the dataSize and numTables loaded */
            me->eosData.dataSize =
                2 + me->NR + me->NT +
                3 * me->NR * me->NT; /* only THREE subtables */
      ...
      } /* end (tableNum == 301 || tableNum == 303 || tableNum == 304 ||
           tableNum == 305 || tableNum == 306) */

      /* All other data (i.e., 411 and 412 sesame tables) require no additional
       * modifications or calculations, and it is already loaded into the
       * read_data[] array by eos_LoadSesameFiles() above.
       */

      // store the number of subTables stored in memory
      me->eosData.numSubtablesLoaded = (EOS_INTEGER)MIN(
          MAX_TABLES_RECORDTYPE1,
...
#ifdef ENABLE_COLD_CURVE_REMOVAL
  /* if table is 301 or 303, and coldcurve arrays are allocated, subtract cold
   * curve for all tables */
  for (j = 0; j < me->NT; j++) {
    for (i = 0; i < me->NR; i++) {
      ...
      if ((tableNum == 301 || tableNum == 303) && tableNum != 306 &&
          ...
          me->coldCurve4) {
        if (j == 0) {
          me->coldCurve1[i] = (me->table1 && me->eosData.coldCurveIsLoaded)
                                  ? me->table1[0][i]
                                  : ZERO;
          me->coldCurve2[i] = (me->table2 && me->eosData.coldCurveIsLoaded)
                                  ? me->table2[0][i]
                                  : ZERO;
          me->coldCurve3[i] = (me->table3 && me->eosData.coldCurveIsLoaded)
                                  ? me->table3[0][i]
                                  : ZERO;
          me->coldCurve4[i] = (me->table4 && me->eosData.coldCurveIsLoaded)
                                  ? me->table4[0][i]
                                  : ZERO;
        }
...
/***********************************************************************/
/*!
 * \brief This helper function returns a pointer to a new eos_RecordType1
 *object, which contains a copy of the data stored in me.
 *
 * \param[in,out] *me        - eos_RecordType1 : data object pointer;
 *                             contents are populated with data
 * \param[in]     dataType   - EOS_INTEGER : data type
 *
 * \return EOS_INTEGER : table handle associated with new_me (th < 0 if error
 *occurs)
 *
 ***********************************************************************/
EOS_INTEGER _eos_createObjectDataCopyRecordType1(eos_RecordType1 *me,
  ...
  */
  ...
  /* first construct an empty object w/o creating it (in case the table can't be
   * created, we still want an empty object */
  eos_CreateTablesEosDataMap(&gEosDataMap, 1, &dataType,
                             &me->eosData.materialID, &th, EOS_FALSE, 0,
                             EOS_FALSE, -1, &err);

  if (eos_GetStandardErrorCodeFromCustomErrorCode(err) == EOS_OK && th >= 0 &&
      gEosDataMap.tableHandlesMap[th]) {

    /* all is well so far */

    new_me = (eos_RecordType1 *)
                 gEosDataMap.dataObjects[gEosDataMap.tableHandlesMap[th]];
    ...
    for (i = 0; i < MAX_TABLES_RECORDTYPE1; i++) {
      ...
      _eos_GetDataRecordType1(new_me, &new_R, &new_T, &new_F, &new_coldCurve,
                              i + 1);
 ...
 *
 ***********************************************************************/
void _eos_resizeRecordType1(eos_RecordType1 *me, EOS_INTEGER NR, EOS_INTEGER NT,
                            EOS_INTEGER *err) {
  EOS_INTEGER oldNT, oldNR, i, j, k;
  EOS_REAL *oldR, *oldT, **oldF[MAX_TABLES_RECORDTYPE1],
      *oldColdCurve[MAX_TABLES_RECORDTYPE1];
  EOS_REAL *R, *T, **F, *coldCurve;
  EOS_REAL *ptr1;
  *err = EOS_OK;
 ...
 *
 ***********************************************************************/
void _eos_importTableRecordType1(eos_RecordType1 *me, EOS_INTEGER subTableNum,
                                 EOS_INTEGER NR, EOS_INTEGER NT, EOS_REAL *xtbl,
                                 EOS_REAL *ytbl, EOS_REAL *ftbl, EOS_REAL *CC) {
  EOS_INTEGER j, k;
  EOS_REAL *R, *T, **F, *coldCurve;
...
/***********************************************************************/
/*!
 * \brief This helper function iteratively inverts the table with respect to the
 *first independent variable as required by the specified dataType. This
 *algorithm operates on each y-chore (i.e., isotherm) in turn.
 *        Forced-monotonicity is assumed to be already applied to the tables in
 *me.
 *
 * \param[in,out] *me        - eos_RecordType1 : data object pointer;
 *                             contents are populated with data
 * \param[in]     th         - EOS_INTEGER : table handle
 * \param[in]     dataType   - EOS_INTEGER : data type
 * \param[in]         refy   - EOS_REAL : value to find in ytbl[] to determine
 *new first independent variable \param[out]   errorCode   - EOS_INTEGER : error
 *code
 *
 * \return none
 *
 ***********************************************************************/
void _eos_InvertAtSetupRecordType1_CATEGORY1(eos_RecordType1 *me,
                                             EOS_INTEGER th,
                                             EOS_INTEGER dataType,
                                             EOS_REAL refy,
                                             EOS_INTEGER *errorCode) {
  ...
  *errorCode = EOS_OK;
  ...
  _eos_GetInvertedTable(th, dataType, NULL, NULL, NULL, NULL, &xtbl, &ytbl,
                        ftbl, coldCurve, &nxtbl, &nytbl, &xtbl_new, &ytbl_new,
                        &ftbl_new, &ftbl_invt_mask, &err, target_N);
  assert(target_N == nxtbl);
  assert(nytbl == me->NT);

  /* reallocate memory in me and store new data in me */
  _eos_importTableRecordType1(me, subTableNum, nxtbl, nytbl, xtbl_new, ytbl_new,
                              ftbl_new, NULL);
...
/*!
 * \brief This helper function iteratively inverts the table with respect to the
 *second independent variable as required by the specified dataType. This
 *algorithm operates on each x-chore (i.e., isochore) in turn.
 *        Forced-monotonicity is assumed to be already applied to the tables in
 *me.
 *
 * \param[in,out] *me        - eos_RecordType1 : data object pointer;
 ...
 * \param[in]     th         - EOS_INTEGER : table handle
 * \param[in]     dataType   - EOS_INTEGER : data type
 * \param[in]         refx   - EOS_REAL : value to find in xtbl[] to determine
 *new second independent variable \param[out]   errorCode   - EOS_INTEGER :
 *error code
 *
 * \return none
 *
 ***********************************************************************/
void _eos_InvertAtSetupRecordType1_CATEGORY2(eos_RecordType1 *me,
                                             EOS_INTEGER th,
                                             EOS_INTEGER dataType,
                                             EOS_REAL refx,
                                             EOS_INTEGER *errorCode) {
...
/***********************************************************************/
/*!
 * \brief This helper function merges data of a previously inverted CATEGORY1
 *table as required by the specified CATEGORY3 dataType. This algorithm operates
 *on each x-chore (i.e., isochore) in turn. Forced-monotonicity is assumed to be
 *already applied to the tables in me.
 *
 * \param[in,out] *me        - eos_RecordType1 : data object pointer;
 *                             contents are populated with data
 * \param[in]     th         - EOS_INTEGER : table handle
 ...
 *
 ***********************************************************************/
void _eos_InvertAtSetupRecordType1_CATEGORY3(eos_RecordType1 *me,
                                             EOS_INTEGER th,
                                             EOS_INTEGER dataType,
                                             EOS_INTEGER *errorCode) {
  EOS_REAL *xtbl, *ytbl, **ftbl, *coldCurve;
  ...
  *errorCode = EOS_OK;
   ...
   */
  _eos_GetDataRecordType1(me, &_xtbl2, &_ytbl2, &_ftbl2, &_coldCurve2,
                          subTableNum2);
  ...
  /* create inverted table */
  _eos_GetInvertedTable(th, dataType, _xtbl2, _ytbl2, *_ftbl2, _coldCurve2,
                        &xtbl, &ytbl, ftbl, coldCurve, &nxtbl, &nytbl,
                        &xtbl_new, &ytbl_new, &ftbl_new, &ftbl_invt_mask, &err,
                        target_N);
  assert(target_N == nxtbl);
  assert(nytbl == me->NT);

  /* reallocate memory in me and store new data in me */
  _eos_importTableRecordType1(me, subTableNum_new, nxtbl, nytbl, xtbl_new,
                              ytbl_new, ftbl_new, NULL);
...
/***********************************************************************/
/*!
 * \brief This helper function merges data of a previously inverted CATEGORY2
 *table as required by the specified CATEGORY4 dataType. This algorithm operates
 *on each x-chore (i.e., isochore) in turn. Forced-monotonicity is assumed to be
 *already applied to the tables in me.
 *
 * \param[in,out] *me        - eos_RecordType1 : data object pointer;
 ...
 *
 ***********************************************************************/
void _eos_InvertAtSetupRecordType1_CATEGORY4(eos_RecordType1 *me,
                                             EOS_INTEGER th,
                                             EOS_INTEGER dataType,
                                             EOS_INTEGER *errorCode) {
  EOS_REAL *xtbl, *ytbl, **ftbl, *coldCurve;
 ...
 * \param[in,out] *me        - eos_RecordType1 : data object pointer;
 *                                               contents of object are
 *initialized \param[in]     dataType   - EOS_INTEGER : data type
 *
 * \return none
 *
 ***********************************************************************/
void eos_SetExtrapolationBoundsRecordType1(void *ptr, EOS_INTEGER dataType) {
  ...
  optVal = _eos_getOptionEosData(&(me->eosData), EOS_USE_TAYLOR_FIT);
  if (optVal->bval)
    return; /* no tablulated data is loaded to use for bounds */
  ...
  case EOS_CATEGORY0: /* indicates the table is not inverted */
  {
    ...
    me->extrapolationBounds.x = NULL;
    me->extrapolationBounds.xLo =
        (EOS_REAL *)malloc(me->extrapolationBounds.nx * sizeof(EOS_REAL));
    me->extrapolationBounds.yLo =
        (EOS_REAL *)malloc(me->extrapolationBounds.ny * sizeof(EOS_REAL));
    me->extrapolationBounds.xHi =
        (EOS_REAL *)malloc(me->extrapolationBounds.nx * sizeof(EOS_REAL));
    me->extrapolationBounds.yHi =
        (EOS_REAL *)malloc(me->extrapolationBounds.ny * sizeof(EOS_REAL));
  ...
  case EOS_CATEGORY3: /* indicates the table is a merging of a CATEGORY1 table
                         and a CATEGORY0 table */
  {
  ...
  case EOS_CATEGORY1: /* indicates the table is inverted with respect to 1st
  ...
  case EOS_CATEGORY4: /* indicates the table is a merging of a CATEGORY2 table
                         and a CATEGORY0 table */
  {
    /* get the data pointers and types for these variables */
    tabInd2 = EOS_EOS_TABLE_TYPE_REF2(dataType);
  ...
  case EOS_CATEGORY2: /* indicates the table is inverted with respect to 2nd
                         independent variable */
  {
    me->extrapolationBounds.nx = 1;
...
/***********************************************************************/
/*!
 * \brief This function inverts the tables as required by the specified
 *dataType. All tables in me must be inverted since the independent variable
 *changes. Forced-monotonicity is assumed to be already applied to the tables in
 *me.
 *
 * \param[in,out] *ptr       - void : data object pointer;
 *                             internally recast to eos_RecordType1*
 ...
 *
 ***********************************************************************/
void eos_InvertAtSetupRecordType1(void *ptr, EOS_INTEGER th,
                                  EOS_INTEGER dataType,
                                  EOS_INTEGER *errorCode) {
  eos_RecordType1 *me;
  EOS_INTEGER subTableNum = EOS_TYPE_TO_SUB_TAB_NUM(dataType);
  ...
  *errorCode = EOS_OK;
  ...
  case EOS_CATEGORY1: /* indicates the table is inverted with respect to 1st
                         independent variable */
  {
    /* for each isotherm, solve D(f,T) -- where f = P, U, A or S -- given f
     * values selected at or near the ??? */
    _eos_InvertAtSetupRecordType1_CATEGORY1(me, th, dataType, (EOS_REAL)0,
                                            errorCode);
  ...
  case EOS_CATEGORY2: /* indicates the table is inverted with respect to 2nd
                         independent variable */
  {
    /* for each isochore, solve T(D,f) -- where f = P, U, A or S -- given f
     * values selected at or near the normal D */
    _eos_InvertAtSetupRecordType1_CATEGORY2(me, th, dataType, me->refDensity,
                                            errorCode);
    ...
    if (subTableNum > 1) { /* store new dependent values in table1 */
      ...
      _eos_GetDataRecordType1(me, &xtbl, &ytbl, &ftbl, &coldCurve, subTableNum);
      _eos_GetDataRecordType1(me, &xtbl_new, &ytbl_new, &ftbl_new, &coldCurve,
                              1);
 ...
 *
 ***********************************************************************/
void eos_SetSizeRecordType1(eos_RecordType1 *me, EOS_INTEGER NR, EOS_INTEGER NT,
  ...
  /* memory is allocated continuously in one chunk for R, T and all tables */
  ptr = (EOS_REAL *)malloc(sizeof(EOS_REAL) *
                           (NT + NR + MAX_TABLES_RECORDTYPE1 * NT * NR));
  me->R = ptr;
  me->T = ptr + NR;
  ptr1 = ptr + NT + NR;
  ptr2 = ptr1 + NT * NR;
 ...
 * \param[out]    ***F        - EOS_REAL : holds F-pointer (F is 2d array
 *allocated as 1 d array NR * NT) \param[out]    **coldCurve - EOS_REAL : holds
 *cold curve-pointer \param[in]     subtableNum - EOS_INTEGER : number of
 *subtable to take the data from.
 *
 * \return none
 *
 ***********************************************************************/
...
void eos_PrintRecordType1(void *ptr, EOS_INTEGER th, EOS_CHAR *fname,
  ...
  EOS_REAL xprnt, yprnt, fprnt, *coldCurve = NULL, *xtbls = NULL, *ytbls = NULL,
                                *ftbls = NULL, xconv, yconv, fconv;
  EOS_REAL *_xtbls = NULL, *_ytbls = NULL, **_ftbls = NULL, *_coldCurve = NULL;
  EOS_REAL *_xtbls2 = NULL, *_ytbls2 = NULL, **_ftbls2 = NULL,
           *_coldCurve2 = NULL, *f_refData = NULL;
  ...
  *err = EOS_OK;
...
#ifdef DEBUG_EOS_PRINTRECORDTYPE1
  if (!use_taylor_fit) {
    printf("------ eos_PrintRecordType1\nmat=%i th=%i subtab=%i type=%s "
           "table1, table2, table3 and table4\n",
           me->eosData.materialID, th, subTableNum,
           EOS_TYPE_TO_STRING(dataType));
    for (k = 0; k < me->NR; k++)
      for (l = 0; l < me->NT; l++)
        printf("%23.15e %23.15e %23.15e %23.15e %23.15e %23.15e\n", me->R[k],
               me->T[l], me->table1[l][k], me->table2[l][k], me->table3[l][k],
               me->table4[l][k]);
  }
#endif
  ...
  if (use_taylor_fit) {
    ...
    int i;
    EOS_BOOLEAN freeEnergyIsDefined =
        (me->Taylor_objects[2]) ? EOS_TRUE : EOS_FALSE;
    EOS_BOOLEAN isOneDimDatatype = EOS_IS_ONE_DIM_TYPE(dataType);
    ...
    if (me->Taylor_objects[i]) {
      ...
      fprintf(tableFile, "  fconv =%*s         ", _MIN_FIELD_WIDTH + 1, buffer);
...
#define USE_PRINTTAYLORCOEFFICIENTS
      ...
      eos_PrintTaylorCoefficients(me->Taylor_objects[i], me->M, me->N, dataType,
                                  tableFile);
#else
      {
        int j;
      ...
      fprintf(tableFile,
              "%sTableHandle=%i matid =%6d source = "
              "%-20s\n%s%s\n%s%s\nDependent upon the Helmholtz Free Energy "
  ...
  nxtbl = me->NR;
  nytbl = me->NT;
  _eos_GetDataRecordType1(me, &_xtbls, &_ytbls, &_ftbls, &_coldCurve,
                          EOS_TYPE_TO_SUB_TAB_NUM(dataType));
  xtbls = _xtbls;
  ytbls = _ytbls;
  ftbls = *_ftbls;
   ...
   */
  /* EOS_CATEGORY3 -- table is merged with another function to change 1st
   * independent variable */
  /* EOS_CATEGORY4 -- table is merged with another function to change 2nd
   * independent variable */
  ...
  if (EOS_CATEGORY(dataType) != EOS_CATEGORY0 && !me->isInvertedAtSetup) {
    if (EOS_CATEGORY(dataType) == EOS_CATEGORY3 ||
      ...
      if (EOS_TYPE_TO_RECORD_TYPE(EOS_EOS_TABLE_TYPE_REF2(dataType)) ==
        ...
        _eos_GetDataRecordType1(
            me, &_xtbls2, &_ytbls2, &_ftbls2, &_coldCurve2,
            EOS_TYPE_TO_SUB_TAB_NUM(EOS_EOS_TABLE_TYPE_REF2(dataType)));
      } else {
        *err = EOS_UNDEFINED;
        *err = eos_SetCustomErrorMsg(
            th, *err,
            "eos_RecordType1::eos_PrintRecordType1 ERROR, invalid data "
            "RecordType, %i",
      ...
      if (!(_xtbls2 || _ytbls2 || _ftbls2 || _coldCurve2)) {
        *err = eos_SetCustomErrorMsg(th, *err,
                                     "eos_RecordType1::eos_PrintRecordType1 "
                                     "ERROR, temporary array(s) not allocated");
        return;
      }
    ...
    { /* now generate inverted table */
      ...
      default:
        ...
        *err = eos_SetCustomErrorMsg(th, *err,
                                     "eos_RecordType1::eos_PrintRecordType1 "
                                     "ERROR, invalid data category, %i",
                                     EOS_CATEGORY(dataType));
        return;
      }

      _eos_GetInvertedTable(th, dataType, _xtbls2, _ytbls2, f_refData,
                            _coldCurve2, &xtbls, &ytbls, &ftbls, coldCurve,
                            &nxtbl, &nytbl, &xtbl_new, &ytbl_new, &ftbl_new,
                            &ftbls_invt_mask, err, target_N /* -1 */);
      if (eos_GetStandardErrorCodeFromCustomErrorCode(*err) != EOS_OK) {
        *err = eos_SetCustomErrorMsg(
            th, *err,
            "eos_RecordType1::eos_PrintRecordType1 ERROR, "
            "_eos_GetInvertedTable failed to invert table");
        return;
      }
    ...
    /* NULLify coldCurve[] since it is not needed for dumping after
     * interpolation of inverted table */
    coldCurve = NULL;
  ...
  { /* fill ftbls_invt_mask[] with extrapolation error codes */
    ...
    me->eosData.CheckExtrap(ptr, th, dataType, nXYPairs, xVals, yVals,
                            ftbls_invt_mask, err);
  ...
  for (iy = 1; iy <= nytbl; iy += 10) {
    ...
    for (i = 0; i < EOS_TOTAL_TABLE_OPTIONS; i++) {
      ...
      if (!EOS_IS_LOADING_OPTION(
              opt) || /* consider only loading options -- duh! */
          opt == EOS_DUMP_DATA ||
          opt == EOS_APPEND_DATA /* this should be obvious if the tableFile
                                    exists! */
      )
        continue;
 ...
 * \param[in]     *ptr         - void        : data object pointer;
 *                                             internally recast to
 *eos_RecordType1* \param[in]     th           - EOS_INTEGER : table handle
 *
 * \return none
 *
 ***********************************************************************/
void eos_GetPackedTableRecordType1(void *ptr, EOS_INTEGER th,
  ...
  *err = EOS_OK;
  ...
  memcpy(packedTable + byteCount, &(me->isMonotonicX1), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicX2), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicX3), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicX4), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  ...
  memcpy(packedTable + byteCount, &(me->shouldBeMonotonicX4),
         sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicY1), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicY2), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicY3), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->isMonotonicY4), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  ...
  memcpy(packedTable + byteCount, &(me->shouldBeMonotonicY4),
         sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->shouldBeSmooth1), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->shouldBeSmooth2), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->shouldBeSmooth3), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(packedTable + byteCount, &(me->shouldBeSmooth4), sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  ...
  memcpy(packedTable + byteCount, &(me->extrapolationBounds.stored),
         sizeof(EOS_BOOLEAN));
  byteCount += sizeof(EOS_BOOLEAN);

  if (me->extrapolationBounds.stored) {
    memcpy(packedTable + byteCount, &(me->extrapolationBounds.nx),
           sizeof(EOS_INTEGER));
    byteCount += sizeof(EOS_INTEGER);
    memcpy(packedTable + byteCount, &(me->extrapolationBounds.ny),
           sizeof(EOS_INTEGER));
    byteCount += sizeof(EOS_INTEGER);

    tmpINT = MAX(me->extrapolationBounds.nx, me->extrapolationBounds.ny);
    if (tmpINT > 1) { /* not CATEGORY0 */
      memcpy(packedTable + byteCount, me->extrapolationBounds.x,
             tmpINT * sizeof(EOS_REAL));
      byteCount += tmpINT * sizeof(EOS_REAL);
    }

    memcpy(packedTable + byteCount, me->extrapolationBounds.xLo,
           me->extrapolationBounds.nx * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.nx * sizeof(EOS_REAL);

    memcpy(packedTable + byteCount, me->extrapolationBounds.yLo,
           me->extrapolationBounds.ny * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.ny * sizeof(EOS_REAL);

    memcpy(packedTable + byteCount, me->extrapolationBounds.xHi,
           me->extrapolationBounds.nx * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.nx * sizeof(EOS_REAL);

    memcpy(packedTable + byteCount, me->extrapolationBounds.yHi,
           me->extrapolationBounds.ny * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.ny * sizeof(EOS_REAL);
  }
  ...
  byteCount += sizeof(EOS_INTEGER);
  if (me->found_401) {
    tbl401 = (eos_RecordType2 *)gEosDataMap
                 .dataObjects[gEosDataMap.tableHandlesMap[me->rt2_handle]];
...
/***********************************************************************/
/*!
 * \brief This sets the data of class eos_RecordType1 from the packed char array
 *provided by caller
 *
 * \param[out]   *err            - EOS_INTEGER : error code
 * \param[in]    *ptr            - void : data object pointer;
 *                                        internally recast to eos_RecordType1*
 * \param[in]    th              - EOS_INTEGER : table handle
 * \param[in]    packedTableSize - EOS_INTEGER : size in chars of packed data
 *array \param[in]    *packedTable    - EOS_CHAR : allocated by user char array
 *large enough to store packed data
 *
 * \return none
 *
...
void eos_SetPackedTableRecordType1(void *ptr, EOS_INTEGER th,
                                   EOS_CHAR *packedTable, EOS_INTEGER *err) {
  eos_RecordType1 *me;
  EOS_INTEGER dataType, nt, nr, byteCount = 0, tableNum, t, j, dt, tmpINT;
  ...
  *err = EOS_OK;
  ...
  memcpy(&(me->isMonotonicX1), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicX2), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicX3), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicX4), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  ...
  memcpy(&(me->shouldBeMonotonicX4), packedTable + byteCount,
         sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicY1), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicY2), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicY3), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->isMonotonicY4), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  ...
  memcpy(&(me->shouldBeMonotonicY4), packedTable + byteCount,
         sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->shouldBeSmooth1), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->shouldBeSmooth2), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->shouldBeSmooth3), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  memcpy(&(me->shouldBeSmooth4), packedTable + byteCount, sizeof(EOS_INTEGER));
  byteCount += sizeof(EOS_INTEGER);
  ...
  memcpy(&(me->extrapolationBounds.stored), packedTable + byteCount,
         sizeof(EOS_BOOLEAN));
  byteCount += sizeof(EOS_BOOLEAN);

  if (me->extrapolationBounds.stored) {
    memcpy(&(me->extrapolationBounds.nx), packedTable + byteCount,
           sizeof(EOS_INTEGER));
    byteCount += sizeof(EOS_INTEGER);
    memcpy(&(me->extrapolationBounds.ny), packedTable + byteCount,
           sizeof(EOS_INTEGER));
    byteCount += sizeof(EOS_INTEGER);
    ...
    if (tmpINT > 1) { /* not CATEGORY0 */
      ...
      memcpy(me->extrapolationBounds.x, packedTable + byteCount,
             tmpINT * sizeof(EOS_REAL));
      byteCount += tmpINT * sizeof(EOS_REAL);
    } else {
    ...
    me->extrapolationBounds.xLo =
        (EOS_REAL *)malloc(me->extrapolationBounds.nx * sizeof(EOS_REAL));
    memcpy(me->extrapolationBounds.xLo, packedTable + byteCount,
           me->extrapolationBounds.nx * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.nx * sizeof(EOS_REAL);

    me->extrapolationBounds.yLo =
        (EOS_REAL *)malloc(me->extrapolationBounds.ny * sizeof(EOS_REAL));
    memcpy(me->extrapolationBounds.yLo, packedTable + byteCount,
           me->extrapolationBounds.ny * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.ny * sizeof(EOS_REAL);

    me->extrapolationBounds.xHi =
        (EOS_REAL *)malloc(me->extrapolationBounds.nx * sizeof(EOS_REAL));
    memcpy(me->extrapolationBounds.xHi, packedTable + byteCount,
           me->extrapolationBounds.nx * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.nx * sizeof(EOS_REAL);

    me->extrapolationBounds.yHi =
        (EOS_REAL *)malloc(me->extrapolationBounds.ny * sizeof(EOS_REAL));
    memcpy(me->extrapolationBounds.yHi, packedTable + byteCount,
           me->extrapolationBounds.ny * sizeof(EOS_REAL));
    byteCount += me->extrapolationBounds.ny * sizeof(EOS_REAL);
  }
  ...
  for (t = 1; t <= me->eosData.numSubtablesLoaded; t++) {
    ...
    case 1:
      ...
      if (!me->eosData
               .coldCurveIsLoaded) /* Cold curve is not stored separately */
        EOS_FREE(me->coldCurve1);
      break;
    case 2:
      F = &(me->table2[0][0]);
  ...
  if (me->found_401) {
    ...
    dt = EOS_401_DATA;
    /* first construct an empty tbl w/o creating it (in case the table can't be
     * created, we still want an empty RT2 */
    eos_CreateTablesEosDataMap(&gEosDataMap, 1, &dt, &me->eosData.materialID,
                               &(me->rt2_handle), EOS_FALSE, 0, EOS_FALSE, -1,
                               err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*err) == EOS_OK &&
        me->rt2_handle >= 0 && gEosDataMap.tableHandlesMap[me->rt2_handle]) {
      tbl401 = (eos_RecordType2 *)gEosDataMap
                   .dataObjects[gEosDataMap.tableHandlesMap[me->rt2_handle]];
      ...
      *err = EOS_UNDEFINED;
      *err = eos_SetCustomErrorMsg(th, *err,
                                   "eos_RecordType1::SetPackedTable ERROR "
                                   "couldn't create a handle for 401 table");
    }
  }
}
...
/***********************************************************************/
/*!
 * \brief This returns the size of packed char array needed to store the data of
 *class eos_RecordType1
 *
 * \param[out]   *err             - EOS_INTEGER : error code
 * \param[in]    *ptr             - void : data object pointer;
 *                                         internally recast to eos_RecordType1*
 * \param[in]    *packedTableSize - EOS_INTEGER : size in chars of packed data
 *array \param[in]    th               - EOS_INTEGER : table handle
 *
 * \return none
 *
 ***********************************************************************/
void eos_GetPackedTableSizeRecordType1(void *ptr, EOS_INTEGER th,
  ...
  *err = EOS_OK;
  ...
  if (me->extrapolationBounds.stored) {
    byteCount += sizeof(EOS_INTEGER) *
                 2; /* extrapolationBounds.nx and extrapolationBounds.ny */
    tmpINT = MAX(me->extrapolationBounds.nx, me->extrapolationBounds.ny);
    if (tmpINT > 1) { /* not CATEGORY0 */
      byteCount +=
          tmpINT * sizeof(EOS_REAL); /* extrapolationBounds.x[] array */
    }
    byteCount += me->extrapolationBounds.nx *
                 sizeof(EOS_REAL); /* extrapolationBounds.xLo[] array */
    byteCount += me->extrapolationBounds.ny *
                 sizeof(EOS_REAL); /* extrapolationBounds.xLo[] array */
    byteCount += me->extrapolationBounds.nx *
                 sizeof(EOS_REAL); /* extrapolationBounds.xHi[] array */
    byteCount += me->extrapolationBounds.ny *
                 sizeof(EOS_REAL); /* extrapolationBounds.yHi[] array */
  }

  byteCount += sizeof(EOS_INTEGER); /* 401 handle */

  byteCount +=
      me->eosData.numSubtablesLoaded * me->NT * me->NR * sizeof(EOS_REAL);

  if (me->eosData.coldCurveIsLoaded)
    byteCount += me->eosData.numSubtablesLoaded * me->NR *
                 sizeof(EOS_REAL); /* cold curves for each subtable */
 ...
 *
 * \param[out]   fVals[nXYPairs] - EOS_REAL : array of the interpolated data
 *corresponding to x and y. \param[out]   dFx[nXYPairs]   - EOS_REAL : array of
 *the interpolated partial derivatives of fVals with respect to x. \param[out]
 *dFy[nXYPairs]   - EOS_REAL : array of the interpolated partial derivatives of
 *fVals with respect to y. \param[out]   dFCx[nXYPairs]  - EOS_REAL : optional
 *array of the interpolated partial derivatives of cold curve with respect to x.
 * \param[out]   dFx0[nXYPairs]  - EOS_REAL : optional array of the interpolated
 *partial derivatives of cat 0 F wrt x (for inverse functions mostly).
 * \param[out]   dFy0[nXYPairs]  - EOS_REAL : optional array of the interpolated
 *partial derivatives of cat 0 F wrt y (for inverse functions mostly).
 * \param[out]   *xyBounds       - EOS_INTEGER : interpolation errors per
 *xy-pair \param[out]   errorCode       - EOS_INTEGER : error code of the
 *interpolation: EOS_INTERP_EXTRAPOLATED or EOS_OK \param[in]    *ptr - void :
 *data object pointer; internally recast to eos_RecordType1* \param[in]    th -
 *EOS_INTEGER : table Handle \param[in]    varOrder        - EOS_INTEGER : order
 *of variables \param[in]    dataType        - EOS_INTEGER : dataType \param[in]
 *nXYPairs        - EOS_INTEGER : total number of pairs of independent variable
 *values provided for interpolation. \param[in]    srchX[nXYPairs] - EOS_REAL :
 *array of the primary independent variable values to use during interpolation.
 * \param[in]    srchY[nXYPairs] - EOS_REAL : array of the secondary independent
 *variable values to use during interpolation.
 *
 * \return none
 *
 ***********************************************************************/
void _eos_InterpolateRecordType1(
    void *ptr, EOS_INTEGER th, EOS_INTEGER varOrder, EOS_INTEGER dataType,
  ...
  EOS_INTEGER i, ind, doRational = 0, err, tabInd1, tabInd2, nX, nY, cat,
                      subTableNum, subTableNum2, tableNum, *xyBounds2;
  EOS_REAL *X, *Y, **F, **F2, *uVals, *dUy, *dUx, *dFCx1, *dUCx, *dYx, *dXy,
      *coldCurve, *newColdCurve = NULL, *newColdCurve_dFdx = NULL, *newFVals,
                  *xVals, *yVals, *null_val = NULL;
  eos_Data *eosData;
  eos_RecordType1 *me;
  EOS_BOOLEAN isPtSmooth = EOS_FALSE, useCustomInterp = EOS_FALSE;
  EOS_INTEGER nGhostData = 0;

  EOS_REAL *xtbls, *ytbls,
      **ftbls; /* temporary arrays to include ghost node data */
  ...
  *errorCode = EOS_OK;
  ...
  /* note, when the data is log 10 - cold curve, we need to take the indep. var
   * into this form when inverting */
  tableNum = EOS_TYPE_TO_TAB_NUM(dataType);
  ...
  if (isPtSmooth && useCustomInterp &&
    ...
    _eos_SesameInvTRecordType1(me, dataType, nXYPairs, srchX, srchY, fVals, dFx,
                               dFy, xyBounds, errorCode, &errMsg);
    if (errMsg)
  ...
  eos_GetOptionEosInterpolation(&gEosInterpolation, th, EOS_DISABLE_GHOST_NODES,
                                &optVal, errorCode);
  if (eos_GetStandardErrorCodeFromCustomErrorCode(*errorCode) != EOS_OK) {
    ((eos_ErrorHandler *)me)->HandleError(me, th, *errorCode);
    return;
  ...
  if (!optVal && doRational && (nX > 1) &&
      (nY > 1) /*  && (cat != EOS_CATEGORY0) */) {
    ...
    if (EOS_TYPE_TO_INDEP_VAR1(dataType) != EOS_NullTable &&
      ...
      _eos_CreateGhostData(nGhostData, nX, nY, X, NULL, F, NULL, &nxtbl, &nytbl,
                           &xtbls, &ytbls, &ftbls, NULL, &err, &errMsg);
      if (errMsg)
        err = eos_SetCustomErrorMsg(th, err, "%s", errMsg);
      EOS_FREE(errMsg);
    } else {
      /* operating on a 2-D table */
      _eos_CreateGhostData(nGhostData, nX, nY, X, Y, F, NULL, &nxtbl, &nytbl,
                           &xtbls, &ytbls, &ftbls, NULL, &err, &errMsg);
      if (errMsg)
        err = eos_SetCustomErrorMsg(th, err, "%s", errMsg);
      EOS_FREE(errMsg);
  ...
  if (EOS_TYPE_TO_INDEP_VAR1(dataType) != EOS_NullTable &&
    ...
    /* interpolate */
    _eos_InterpolateRecordType_1D(nX, X, *F, *F2, th, dataType, nXYPairs, srchX,
                                  fVals, dFx, xyBounds, errorCode, &errMsg);
    if (errMsg)
      *errorCode = eos_SetCustomErrorMsg(th, *errorCode, "%s", errMsg);
    EOS_FREE(errMsg);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*errorCode) != EOS_OK)
      ((eos_ErrorHandler *)me)->HandleError(me, th, *errorCode);
    return;
    ...
    /* interpolate */
    _eos_InterpolateRecordType_1D(nY, Y, *F, *F2, th, dataType, nXYPairs, srchX,
                                  fVals, dFx, xyBounds, errorCode, &errMsg);
    if (errMsg)
      *errorCode = eos_SetCustomErrorMsg(th, *errorCode, "%s", errMsg);
    EOS_FREE(errMsg);
  ...
  switch (cat) {
  case EOS_CATEGORY0:         /* indicates the table is not inverted */
  case EOS_INVERTED_AT_SETUP: /* indicates the table is already inverted during
                                 setup */
  {
    ...
    if (cat != EOS_INVERTED_AT_SETUP) {
      ...
      if ((tableNum == 301 || tableNum == 303) && coldCurve) {
        ...
        _eos_InterpolateRecordType_1D(
            nX, X, coldCurve, NULL, th, dataType, nXYPairs, xVals, newColdCurve,
            newColdCurve_dFdx, xyBounds, &err, &errMsg);
        if (errMsg)
          err = eos_SetCustomErrorMsg(th, err, "%s", errMsg);
        EOS_FREE(errMsg);
        for (i = 0; i < nXYPairs; i++) {
    ...
    if (doRational) {
      ...
      eos_RationalInterpolateXY(nXYPairs, xVals, yVals, dFx, dFy, fVals, nX, nY,
                                X, Y, F, coldCurve, nGhostData, xyBounds, &err);

    } else if (eos_getBoolOptionFromTableHandle(
                   th, EOS_LINEAR, &err)) { /* interpolate linearly instead */
#ifdef _EOS_DUMP_INDEX_DATA_INTERPOLATE_RECORDTYPE1
      sprintf(fn, "eos_LinearInterpolate_RecordType1.indices");
      iyv = (EOS_INTEGER *)malloc(
          nXYPairs *
          sizeof(EOS_INTEGER)); /* indexes of Y near which Y points are */
      ixv = (EOS_INTEGER *)malloc(
          nXYPairs *
          sizeof(EOS_INTEGER)); /* indexes of X near which X points are */
      xyBounds2 = (EOS_INTEGER *)malloc(
          nXYPairs * sizeof(EOS_INTEGER)); /* xy-bounds for y */

      _eos_srchdf(nXYPairs, yVals, 1, nY - 1, Y, 1, iyv, xyBounds, errorCode);
      _eos_srchdf(nXYPairs, xVals, 1, nX - 1, X, 1, ixv, xyBounds2, errorCode);
      _eos_DumpIndicesToFile(fn, mode, nXYPairs, X, Y, F, ixv, iyv, NULL, NULL);
      EOS_FREE(xyBounds2);
      EOS_FREE(ixv);
      EOS_FREE(iyv);
#endif
      eos_BiLineInterpolate(eos_getBoolOptionFromTableHandle(
                                th, EOS_DISCONTINUOUS_DERIVATIVES, &err),
                            nXYPairs, nX, nY, X, Y, F, xVals, yVals, fVals, dFx,
                            dFy, xyBounds, &err);
    }
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    ...
    if (cat != EOS_INVERTED_AT_SETUP) {
      ...
      for (i = 0; i < nXYPairs; i++) {
        ...
        if ((tableNum == 301 || tableNum == 303) && newColdCurve) {
          // OLgaLog10             fVals[i] = (tableNum == 301 || tableNum ==
          // 303)?  pow(10.0, fVals[i]) + newColdCurve[i] : pow(10.0, fVals[i])
          // ;
          fVals[i] = fVals[i] + newColdCurve[i];
          // DAP -- Add dFcold/dx to dF/dx since interpolator was only using
          // thermal data
  ...
  case EOS_CATEGORY1: /* indicates the table is inverted with respect to 1st
                         independent variable */
  {
    /* means that the user provides y and F of a function that is defined as x(
       F,y) so effectively the x and y user has given us are F and Y */
    ...
    if (doRational) {
      err = EOS_OK;
      eos_InverseRationalInterpolateFY(nXYPairs, yVals, xVals, dFx, dFy, fVals,
                                       nX, nY, X, Y, F, coldCurve, nGhostData,
                                       xyBounds, &err, &errMsg);
      if (errMsg)
        err = eos_SetCustomErrorMsg(th, err, "%s", errMsg);
      EOS_FREE(errMsg);
                   ...
                   th, EOS_LINEAR, &err)) /* interpolate linearly instead */
      eos_InverseBilinearInterpolateFY(nXYPairs, yVals, xVals, dFx, dFy, fVals,
                                       nX, nY, X, Y, F, coldCurve, xyBounds,
                                       &err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) == EOS_UNDEFINED)
    ...
    for (i = 0; i < nXYPairs; i++) {
      ...
      /* DAP -- Test multiplication with EOS_CHECK_PRODUCT prior to performing
         it. This is a temporary kludge to prevent SIGFPE. */
      /* dFy in this case needs to be transformed to dx/dy */
      if (EOS_CHECK_PRODUCT(dFy[i], -ONE / dFx[i]))
        dFy[i] = (-ONE / dFx[i]) * dFy[i];
      else
        dFy[i] =
            SIGN(dFy[i]) * SIGN(-ONE / dFx[i]) *
            (EOS_IS_PRODUCT_GT_MAX(dFy[i], -ONE / dFx[i]) ? DBL_MAX : DBL_MIN);

      /*calculate dx/dF*/
      dFx[i] = ONE / dFx[i];

      /* compute EOS_V_PtT differently: v = 1/r, dv/dP = -(1/r^2) * dr/dP =
       * -1/xVals[i]^2 * dFx[i] */
        ...
        if (EOS_CHECK_PRODUCT(fVals[i], fVals[i]) &&
          ...
          dFx[i] = -ONE * SIGN(dFx[i]) *
                   (EOS_IS_PRODUCT_GT_MAX(fVals[i], fVals[i]) ||
                            EOS_IS_PRODUCT_GT_MAX(fVals[i] * fVals[i], dFx[i])
                        ? DBL_MAX
                        : DBL_MIN);
      }
    }
                         ...
                         independent variable */
  {
    ...
    /* means that the user provides x and F of a function that is defined as y(
       x,F) so effectively the x and y user has given us are X and F */
    ...
    if ((tableNum == 301 || tableNum == 303) && coldCurve) {
      ...
      if (doRational)
        eos_RationalInterpolate(nXYPairs, nX, 1, 0, X, coldCurve, xVals,
                                newColdCurve, newColdCurve_dFdx, 'y', xyBounds,
                                &err);
      else if (eos_getBoolOptionFromTableHandle(
                   th, EOS_LINEAR, &err)) { /* interpolate linearly instead */
        eos_LineInterpolate(eos_getBoolOptionFromTableHandle(
                                th, EOS_DISCONTINUOUS_DERIVATIVES, &err),
                            nXYPairs, nX, 1, 0, X, &coldCurve, xVals,
                            newColdCurve, newColdCurve_dFdx, 'y', xyBounds,
                            &err);
        for (i = 0; i < nXYPairs; i++) {
          if (xyBounds[i] == EOS_UNDEFINED) {
    ...
    /* need to take yVals which are in fact F-vals into the same form as stored
     * in the table */
    newFVals = (EOS_REAL *)malloc(nXYPairs * sizeof(EOS_REAL));
    for (i = 0; i < nXYPairs; i++)
      // OlgaLog10    newFVals[i] = (tableNum == 301 || tableNum == 303)?
      // log10(yVals[i] - newColdCurve[i]) : log10(yVals[i]);
      newFVals[i] = ((tableNum == 301 || tableNum == 303) && newColdCurve)
                        ? yVals[i] - newColdCurve[i]
                        : yVals[i];
    ...
    if (doRational) {
      ...
      eos_InverseBilinearInterpolateXF(nXYPairs, xVals, newFVals, dFx, dFy,
                                       fVals, nX, nY, X, Y, F, xyBounds, &err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) == EOS_UNDEFINED) {
      if ((tableNum == 301 || tableNum == 303) && newColdCurve) {
    ...
    for (i = 0; i < nXYPairs; i++) {
      ...
      if ((tableNum == 301 || tableNum == 303) && newColdCurve) {
        ...
        if (EOS_CHECK_PRODUCT(-(dFx[i] + newColdCurve_dFdx[i]), dFy[i]))
          ...
          dFx[i] =
              -(SIGN(dFx[i] + newColdCurve_dFdx[i])) * SIGN(dFy[i]) *
              (EOS_IS_PRODUCT_GT_MAX(-(dFx[i] + newColdCurve_dFdx[i]), dFy[i])
                   ? DBL_MAX
                   : DBL_MIN);
  ...
  case EOS_CATEGORY4: /* indicates the table is merged with another function */
  {
    ...
    _eos_InterpolateRecordType1(eosData, th, X_U_F, tabInd2, nXYPairs, xVals,
                                yVals, uVals, dFx /*dUx */, dUy, dUCx, dYx,
                                NULL, xyBounds, &err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*errorCode) ==
        EOS_UNDEFINED) {
      EOS_FREE(dUy);
      EOS_FREE(uVals);
      EOS_FREE(dUCx);
    ...
    _eos_InterpolateRecordType1(eosData, th, X_Y_U, tabInd1, nXYPairs, xVals,
                                uVals, fVals, dFx, dFy /*dFu */, dFCx1, NULL,
                                NULL, xyBounds2, &err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*errorCode) ==
  ...
  case EOS_CATEGORY3: /* indicates the table is merged with another function */
  {
    ...
    xyBounds2 = (EOS_INTEGER *)malloc(nXYPairs * sizeof(EOS_INTEGER));
    _eos_InterpolateRecordType1(eosData, th, U_Y_F, tabInd2, nXYPairs, xVals,
                                yVals, uVals, dUx, dFy /*dUy */, NULL, NULL,
                                dXy, xyBounds, &err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*errorCode) ==
    ...
    _eos_InterpolateRecordType1(eosData, th, X_Y_U, tabInd1, nXYPairs, uVals,
                                yVals, fVals, dFx /*dFu */, dFy, NULL, NULL,
                                NULL, xyBounds2, &err);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(err) != EOS_OK)
      *errorCode = err;
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*errorCode) ==
...
EOS_INTEGER _eos_EvaluateTransformedTaylorFor_Pt_DT(eos_Taylor *T, EOS_REAL x,
...
EOS_INTEGER _eos_EvaluateTransformedTaylorFor_Ut_DT(eos_Taylor *T, EOS_REAL x,
                                                    EOS_REAL y, EOS_REAL *f,
                                                    EOS_REAL *dFx,
                                                    EOS_REAL *dFy) {
  /* initialize */
  ...
  *dFx = -(x * x) * (T->DerivativeScalar(T, 1, 0, x, y) -
                     y * T->DerivativeScalar(T, 1, 1, x, y));
  *dFy = -y * T->DerivativeScalar(T, 0, 2, x, y);

  return EOS_OK;
}

EOS_INTEGER _eos_EvaluateTaylorFor_dataType_DT(eos_Taylor *T, EOS_REAL x,
                                               EOS_REAL y, EOS_REAL *f,
...
EOS_INTEGER _eos_EvaluateTransformedTaylorFor_St_DT(eos_Taylor *T, EOS_REAL x,
                                                    EOS_REAL y, EOS_REAL *f,
                                                    EOS_REAL *dFx,
                                                    EOS_REAL *dFy) {
  /* initialize */
 ...
 *interpolation: EOS_INTERP_EXTRAPOLATED or EOS_OK \param[in]    *ptr - void :
 *data object pointer; internally recast to eos_RecordType1* \param[in]    th -
 *EOS_INTEGER : table Handle \param[in]    dataType        - EOS_INTEGER :
 *dataType \param[in]    nXYPairs        - EOS_INTEGER : total number of pairs
 *of independent variable values provided for interpolation. \param[in]
 *srchX[nXYPairs] - EOS_REAL : array of the primary independent variable values
 *to use during interpolation. \param[in]    srchY[nXYPairs] - EOS_REAL : array
 *of the secondary independent variable values to use during interpolation.
 *
 * \return none
 *
 ***********************************************************************/
void _eos_EvaluateTaylorRecordType1(
    void *ptr, EOS_INTEGER th, EOS_INTEGER dataType, EOS_INTEGER nXYPairs,
  ...
  if (my_subTableNum >= 0 && *(me->_eos_EvaluateTaylor)[my_subTableNum]) {
    ...
    /* Evaluate Taylor polynomial (currently only for EOS_Pt, EOS_Ut, EOS_At, or
     * EOS_St functions) */
    ...
    for (i = 0; i < nXYPairs;
         i++) { /* invert X values for search; impose TINY_D as minimum */
      inverse_srchX[i] = 1.0 / MAX(srchX[i], TINY_D);
    }

    _eos_srchdf(nXYPairs, inverse_srchX, 1, me->M, me->TX, 1, ix_low, xyBounds,
                &err);
    ...
    for (i = 0; i < nXYPairs; i++) {
      ...
      err = (*(me->_eos_EvaluateTaylor)[my_subTableNum])(
          T, inverse_srchX[i], srchY[i], &fVals[i], &dFx[i], &dFy[i]);
    }

    if (!err) {
      /* fix xyBounds[] array content since ranges in me->TX and/or me->TY do
       * not represent to original data ranges */
      ...
      *errorCode = _eos_CheckExtrapEosInterpolationGeneric(
          xMin, xMax, yMin, yMax, nXYPairs, srchX, srchY, xyBounds);
    } else {
      *errorCode = err;
    }
 ...
 *the interpolated partial derivatives of fVals with respect to x. \param[out]
 *dFy[nXYPairs]   - EOS_REAL : array of the interpolated partial derivatives of
 *fVals with respect to y. \param[out]   *xyBounds       - EOS_INTEGER :
 *interpolation errors per xy-pair \param[out]   errorCode       - EOS_INTEGER :
 *error code of the interpolation: EOS_INTERP_EXTRAPOLATED or EOS_OK
 *
 * The input arguments are:
 * void *ptr                 a pointer to a eos_RecordType1 instance from which
 *to extract the data. EOS_INTEGER th            table Handle EOS_INTEGER
 *dataType      dataType EOS_INTEGER nXYPairs      total number of pairs of
 *independent variable values provided for interpolation. EOS_REAL
 *srchX[nXYPairs]  array of the primary independent variable values to use
 *during interpolation. EOS_REAL srchY[nXYPairs]  array of the secondary
 *independent variable values to use during interpolation.
 *
 * \return none
 *
 ***********************************************************************/
void eos_InterpolateRecordType1(void *ptr, EOS_INTEGER th, EOS_INTEGER dataType,
                                ...
                                EOS_REAL *srchY, EOS_REAL *fVals, EOS_REAL *dFx,
  ...
  if (optVal->bval) {
    ...
    _eos_EvaluateTaylorRecordType1(ptr, th, dataType, nXYPairs, srchX, srchY,
                                   fVals, dFx, dFy, NULL, NULL, NULL, xyBounds,
                                   errorCode);
    ...
    if (me->isInvertedAtSetup)
      eos_SetOptionEosInterpolation(
          &gEosInterpolation, th, EOS_DISABLE_GHOST_NODES, EOS_TRUE, errorCode);

    /* Interpolate tabular data */
    _eos_InterpolateRecordType1(ptr, th, me->eosData.varOrder, dataType,
                                nXYPairs, srchX, srchY, fVals, dFx, dFy, NULL,
                                NULL, NULL, xyBounds, errorCode);
  }
}
 ...
 *
 * The input arguments are
 * xVals	This is an array of the primary independent variable values to
 use during interpolation. There are nXYPairs elements in xVals.
 * yVals	This is an array of the secondary independent variable values to
 use during interpolation. There are nXYPairs elements in yVals.
 *
 * \param[out]   xyBounds  - EOS_INTEGER : This is an array of size nXYPairs
 elements that
 *                                         returns EOS_OK if extrapolation did
 not occur.
 *                                         If extrapolation occurred the
 variable and direction
 *                                         are determined from Table 2.
 * \param[out]   errorCode - EOS_INTEGER : This is a scalar EOS_INTEGER to
 contain an error code.
 *                                         In the case that eos_Mix returned
 *                                         EOS_INTERP_EXTRAPOLATED as an error
 code,
 *                                         an additional series of steps must be
 performed
 *                                         to determine which tableHandle(s)
 correspond to
 *                                         the extrapolation error:
 *
 *                                         -# For each tableHandle sent to
 eos_Mix, call
 *                                            eos_GetErrorCode and, optionally,
 eos_GetErrorMessage.
 *                                         -# For each of these tableHandles,
 call eos_CheckExtrap
 *                                            to determine one of codes listed
 in Table 2.
 *\code
  ...
  *errorCode = EOS_OK;
  ...
  case EOS_CATEGORY0: /* indicates the table is not inverted */
  {
    if (!isOneDimDatatype) {
      ...
      _eos_CheckExtrapCategory0(0, me->NR, me->NT, X, Y, nXYPairs, xVals, yVals,
                                xyBounds, &err);
                         ...
                         independent variable */
  {
    ...
    if (!isOneDimDatatype) {
      ...
      _eos_CheckExtrapCategory1(eos_getBoolOptionFromTableHandle(
                                    th, EOS_DISCONTINUOUS_DERIVATIVES, &err),
                                0, iy_low, doRational, me->NR, me->NT, X, Y, F,
                         ...
                         independent variable */
  {
    ...
    if (!isOneDimDatatype) {
      ...
      _eos_CheckExtrapCategory2(eos_getBoolOptionFromTableHandle(
                                    th, EOS_DISCONTINUOUS_DERIVATIVES, &err),
                                0, ix_low, doRational, me->NR, me->NT, X, Y, F,
                                nXYPairs, xVals, yVals, xyBounds, &err);
      EOS_FREE(ix_low);
  ...
  case EOS_CATEGORY4: {
    ...
    /* We need G(X,Y) given G(X, U) and U(X, Y) (cat3) or G(X,Y) given G(U, Y)
     * and U(X, Y) */
    ...
    eos_CheckExtrapRecordType1(me, th, tabInd2, nXYPairs, xVals, yVals,
                               xyBounds, errorCode);
    break;
  }
  default: {
    ...
    if (!isOneDimDatatype) {
      ...
      if (doRational) {
        eos_RationalInterpolate(nXYPairs, me->extrapolationBounds.nx, 1, 0,
                                me->extrapolationBounds.x,
                                me->extrapolationBounds.xLo, yVals, fValsLo,
                                NULL, 'y', xyBounds, &err);
        eos_RationalInterpolate(nXYPairs, me->extrapolationBounds.nx, 1, 0,
                                me->extrapolationBounds.x,
                                me->extrapolationBounds.xHi, yVals, fValsHi,
                                NULL, 'y', xyBounds, &err);
      } else {
        eos_LineInterpolate(EOS_TRUE, nXYPairs, me->extrapolationBounds.nx, 1,
                            0, me->extrapolationBounds.x,
                            &(me->extrapolationBounds.xLo), yVals, fValsLo,
                            NULL, 'y', xyBounds, &err);
        eos_LineInterpolate(EOS_TRUE, nXYPairs, me->extrapolationBounds.nx, 1,
                            0, me->extrapolationBounds.x,
    ...
    if (!isOneDimDatatype) {
      ...
      if (doRational) {
        eos_RationalInterpolate(nXYPairs, me->extrapolationBounds.ny, 1, 0,
                                me->extrapolationBounds.x,
                                me->extrapolationBounds.yLo, xVals, fValsLo,
                                NULL, 'y', xyBounds, &err);
        eos_RationalInterpolate(nXYPairs, me->extrapolationBounds.ny, 1, 0,
                                me->extrapolationBounds.x,
                                me->extrapolationBounds.yHi, xVals, fValsHi,
                                NULL, 'y', xyBounds, &err);
      } else {
        eos_LineInterpolate(EOS_TRUE, nXYPairs, me->extrapolationBounds.ny, 1,
                            0, me->extrapolationBounds.x,
                            &(me->extrapolationBounds.yLo), xVals, fValsLo,
                            NULL, 'y', xyBounds, &err);
        eos_LineInterpolate(EOS_TRUE, nXYPairs, me->extrapolationBounds.ny, 1,
 ...
 * \param[in]     dataType - EOS_INTEGER : data type of subtable to be made
 *monotonic \param[in]     inX      - EOS_BOOLEAN : make monotonic in x?
...
void eos_MakeMonotonicRecordType1(void *ptr, EOS_INTEGER th,
  ...
  *err = EOS_OK;
  ...
  if (indep == 1 &&
      me->eosData.coldCurveIsLoaded) { /* make coldCurve monotonic */

    if (me->eosData.numSubtablesLoaded >= subTableNum)
      *err = _eos_MakeMonotonic(me->NR, 1, me->R, me->T, coldCurve, indep, NULL,
                                EOS_FALSE);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*err) != EOS_OK) {
      ((eos_ErrorHandler *)me)->HandleError(me, th, *err);
      return;
  ...
  if (me->eosData.numSubtablesLoaded >= subTableNum)
    *err = _eos_MakeMonotonic(me->NR, me->NT, me->R, me->T, table, indep,
                              coldCurve, EOS_FALSE);
...
/***********************************************************************/
/*!
 * \brief checks if the data of class eos_RecordType1 is monotonic. Also returns
 *EOS_FALSE if the data is not increasing or decreasing EXCEPT the case where
 *the data is ALL tTHE SAME!
 *
...
void eos_IsMonotonicRecordType1(void *ptr, EOS_INTEGER dataType,
  ...
  *err = EOS_OK;
  ...
  if (inX && !inY && *isMonotonicX != -1 /* if set */) {
    ...
    *isMonotonic =
        (*isMonotonicY != 0 && *isMonotonicX != 0) ? EOS_TRUE : EOS_FALSE;
    return;
  }
  ...
  for (i = 2;
    ...
    if (me->eosData.numSubtablesLoaded >= subTableNum &&
        (abs(sign - oldSign) > 0 || (sign == 0 && oldSign == 0))) {
      *isMonotonicX = 0;
      break;
    }
    oldSign = sign;
 ...
 *
 * \param[in]     call_index - EOS_INTEGER : integer used as part of output file
 *name \param[in]     *ptr       - void : data object pointer; internally recast
 *to eos_RecordType1* \param[in]     *s         - EOS_CHAR : string used as part
 *of output file name
 *
 * \return none
 *
...
void _eos_DumpData(EOS_INTEGER call_index, void *ptr, EOS_CHAR *s) {
  ...
  if (me->rt2_handle >= 0) {
    altEosData = (eos_RecordType2 *)gEosDataMap
                     .dataObjects[gEosDataMap.tableHandlesMap[me->rt2_handle]];
    nT401 = altEosData->NT;
    P401 = altEosData->P;
    T401 = altEosData->T;
  ...
  if (me->found_401) {
    ...
    for (j = 0; j < nT401; j++)
      fprintf(fh, "%23.14e%23.14e%23.14e%23.14e%23.14e%23.14e\n", T401[j],
              P401[j], RG401[j], RL401[j], EG401[j], EL401[j]);
  }
  fclose(fh);
}
 ...
 *
 * \param[out]   *T                  - EOS_REAL : array of temperature data to
 *be shifted \param[out]   **P                 - EOS_REAL : array of pressure
 *data to be shifted \param[out]   *err                - EOS_INTEGER : error
 *code \param[out]   *errMsg             - EOS_CHAR** : custom error message
 * \param[in]    userDefinedDataFile - EOS_BOOLEAN : is the associated sesame
 *file specified by the user? \param[in]    dataFileIndex       - EOS_INTEGER :
 *index of the sesame file for reading the data \param[in]    nT - EOS_INTEGER :
 *number of temperatures \param[in]    *R                  - EOS_REAL : density
 *data array \param[in]    mat                 - EOS_INTEGER : material ID
 *
 * \return none
 *
 ***********************************************************************/
void _eos_AdjustDataTables(EOS_BOOLEAN userDefinedDataFile,
                           EOS_INTEGER dataFileIndex, EOS_INTEGER nT,
                           EOS_REAL *R, EOS_REAL *T, EOS_REAL **P,
                           EOS_INTEGER mat, EOS_INTEGER *err,
                           EOS_CHAR **errMsg) {
  EOS_INTEGER it;
  ...
  *err = eos_SesGetBulkData(mat, userDefinedDataFile, dataFileIndex,
                            &avgAtomicNumber, &avgAtomicWgt, &refDensity,
                            &solidBulkModulus, &exchangeCoeff, errMsg);
  if (eos_GetStandardErrorCodeFromCustomErrorCode(*err) != EOS_OK) {
    // set custom message here "Could not read solid bulk modulus from 201
    // table!|);
    eos_SetCustomMsg_str(
        errMsg, "%s",
        "eos_RecordType1::eos_GetBulkData ERROR, bulk data not available");
    return;
  }

  if (solidBulkModulus > (EOS_REAL)ZERO) {
    _eos_DEBUG_PRINT("\n Adjusting temperatures: mid, matdef(3) = %8i %12.4e\n",
                     mat, solidBulkModulus);
    for (it = 0; it < nT; it++)
...
void eos_FixTableRecordType1(void *ptr, EOS_INTEGER th, EOS_INTEGER *err) {
  ...
  *err = EOS_OK;
  ...
  if (eos_GetStandardErrorCodeFromCustomErrorCode(*err) == EOS_OK &&
      alt_handle >= 0 && gEosDataMap.tableHandlesMap[alt_handle]) {
    altEosData =
        (eos_RecordType2 *)
            gEosDataMap.dataObjects[gEosDataMap.tableHandlesMap[alt_handle]];
    ...
    *err = eos_GetErrorCodeEosDataMap(&gEosDataMap, alt_handle);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(*err) != EOS_OK) {
      ...
      me->found_401 = EOS_FALSE; /* but can be reset to true by build_dome
                                    called by _eos_FixTable() */
  ...
  _eos_ConvertUnits(_EOS_SESAME_TO_CGS, me->NR, me->NT, me->R, me->T,
                    ...
                    &avgAtomicWgt401, &refDensity401, &adjustVapPres, err,
                    &errMsg);
  if (errMsg)
    *err = eos_SetCustomErrorMsg(th, *err, "%s", errMsg);
  EOS_FREE(errMsg);
  ...
  /* Adjust data tables: do not allow zero density and patch to shift
   * temperatures */
  _eos_AdjustDataTables(me->eosData.userDefinedDataFile,
                        me->eosData.dataFileIndex, me->NT, me->R, me->T,
                        me->table1, me->eosData.materialID, err, &errMsg);
  if (errMsg)
    *err = eos_SetCustomErrorMsg(th, *err, "%s", errMsg);
  EOS_FREE(errMsg);
  ...
  gen401 = me->found_401; /* if not found, it will be generated and set to
                             EOS_TRUE */
  _eos_FixTable(me->NR, me->NT, me->R, me->T, me->table1, me->table2,
                me->eosData.materialID, me->coldCurve1, me->coldCurve2, &nT401,
                &vaporArrayOffset, &avgAtomicNumber401, &avgAtomicWgt401,
                &refDensity401, &P401, &T401, &RG401, &RL401, &EG401, &EL401,
                &AG401, &AL401, table_type, &gen401, adjustVapPres, err,
                &errMsg);
  if (errMsg)
    *err = eos_SetCustomErrorMsg(th, *err, "%s", errMsg);
  EOS_FREE(errMsg);
  ...
  if (nT401 > 0 && !me->found_401) { /* 401 was not found, but generated */
    ...
    gEosDataMap.tableHandlesMap[me->rt2_handle] =
        objectIndex; /* not nessesary, it's never invalidated
    ...
    altEosData->EG = EG401;
    altEosData->EL = EL401;
    /* AG, AL aren't being set by Rage code, allocate and zero them out for
     * consistency */
  ...
  if (nT401 == 0)
    ...
    altEosData->vaporArrayOffset =
        vaporArrayOffset; /* set vaporArrayOffset since handle is valid */
 ...
 *                                 internally recast to eos_RecordType1*
 * \param[in]    th       - EOS_INTEGER : table Handle
 * \param[in]    dataType - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[in]    ptSmooth - EOS_BOOLEAN : is data  of *ptr eos_RecordType1
 *object to be made smooth according to EOS_PT_SMOOTHING option?
 *
 * \return none
 *
 ***********************************************************************/
void eos_MakeSmoothRecordType1(void *ptr, EOS_INTEGER th, EOS_INTEGER dataType,
...
EOS_INTEGER eos_GetAnalyticalEOSFlags(eos_RecordType1 *me, EOS_INTEGER *imodel,
  ...
  *imodel =
      (optVal && optVal->bval == EOS_TRUE) ? EOS_SPLIT_IDEAL_GAS : *imodel;
  optVal->bval = EOS_FALSE;
  optVal = _eos_getOptionEosData(&(me->eosData), EOS_SPLIT_NUM_PROP);
  *imodel = (optVal && optVal->bval == EOS_TRUE) ? EOS_SPLIT_NUM_PROP : *imodel;
  optVal->bval = EOS_FALSE;
 ...
 * \param[in]    *me      - eos_RecordType1 : pointer to the instance of type
 *eos_RecordType1 \param[in]    *Ftot    - EOS_REAL : total EOS datum \param[in]
 **Fc      - EOS_REAL : Cold curve datum (pointer maybe NULL) \param[in]
 *ccLoaded - EOS_INTEGER : Is cold curve stored separately from Ftot?
 *
 * \return eos_SetAdjustedData - EOS_INTEGER : error code
 *
 ***********************************************************************/
EOS_INTEGER eos_SetAdjustedData(eos_RecordType1 *me, EOS_REAL *Fion,
                                EOS_REAL *Ftot, EOS_REAL *Fc,
  ...
  if (me->eosData.tableNum ==
      304) { /* subtract Ion and cold curve from total */
    *Fion = (ccLoaded) ? *Ftot + Fcold - Fion_local : *Ftot - Fion_local;
  }
  if (me->eosData.tableNum ==
      305) { /* constrain consistently with 303 and 304 tableNum */
...
/***********************************************************************/
/*!
 * \brief Compute the ideal-gas pressure, internal energy, entropy, and
 *Helmholtz free energy (monatomic gas assumed)
 *
 * \param[out]   p   - EOS_REAL : ion pressure (Gj/m3); me->NR x me->NT elements
 * \param[out]   e   - EOS_REAL : internal energy/mass (Gj/Mg); me->NR x me->NT
 *elements \param[out]   a   - EOS_REAL : free energy/mass (Gj/Mg); me->NR x
 *me->NT elements \param[out]   s   - EOS_REAL : entropy/mass (Gj/Mg/K); me->NR
 *x me->NT elements
 *
 * \param[in]    *me - eos_RecordType1 : pointer to the instance of type
 *eos_RecordType1 \param[in]    *altEosData - eos_RecordType1 : pointer to the
 *instance of type eos_RecordType1, which contains 301 data
 *
 * \return eos_computeIdealGasData - EOS_INTEGER : error code
 *
 ***********************************************************************/
EOS_INTEGER eos_computeIdealGasData(eos_RecordType1 *me,
                                    eos_RecordType1 *altEosData, EOS_REAL *p,
  ...
  // compute ideal-gas pressure, internal energy, entropy, and Helmholtz free
  // energy (monatomic gas assumed)
  for (i = 0; i < me->NR; i++) {
    for (j = 0; j < me->NT; j++) {

      p[i + j * me->NR] = UNIVERSAL_GAS_CONST / me->avgAtomicWgt *
                          altEosData->T[j] * altEosData->R[i];
      e[i + j * me->NR] = ((EOS_REAL)1.5) * UNIVERSAL_GAS_CONST /
      ...
      if (altEosData->R[i] > ZERO && altEosData->T[j] > ZERO) {
        ...
        /*! - Helmholtz free energy equation taken from T-1's opensesame (was
         * Grizzly). */
        a[i + j * me->NR] =
            -UNIVERSAL_GAS_CONST * altEosData->T[j] / me->avgAtomicWgt *
            ((EOS_REAL)(-7.072343) +
             ((EOS_REAL)(1.5)) * log(me->avgAtomicWgt * altEosData->T[j]) +
             log(me->avgAtomicWgt / altEosData->R[i]));

        /* calculate entropy */
        s[i + j * me->NR] = (altEosData->T[j] <= ZERO)
                                ? ZERO
                                : (e[i + j * me->NR] - a[i + j * me->NR]) /
                                      UNIVERSAL_GAS_CONST / altEosData->T[j] *
                                      me->avgAtomicWgt;
      } else {
        a[i + j * me->NR] = ZERO;
        s[i + j * me->NR] = ZERO;
      ...
      /* Using total 301 table data, calculate appropriate data for requested
       * table: 303, 304 or 305 */
      ierr = eos_SetAdjustedData(me, &p[i + j * me->NR], &prtot[i + j * me->NR],
                                 ((prtot_cc) ? &prtot_cc[i] : &prtot[i]),
                                 ccLoaded);
      ierr = eos_SetAdjustedData(me, &e[i + j * me->NR], &entot[i + j * me->NR],
                                 ((entot_cc) ? &entot_cc[i] : &entot[i]),
                                 ccLoaded);
      if (antot) {
        ierr = eos_SetAdjustedData(
            me, &a[i + j * me->NR], &antot[i + j * me->NR],
            ((antot_cc) ? &antot_cc[i] : &antot[i]), ccLoaded);
      } else { /* A = U - TS; therefore, A = U at T=0 */
        ierr = eos_SetAdjustedData(
            me, &a[i + j * me->NR], &entot[i + j * me->NR],
            ((entot_cc) ? &entot_cc[i] : &entot[i]), ccLoaded);
      }
 ...
 *
 ***********************************************************************/
EOS_INTEGER eos_computeCowanData(eos_RecordType1 *me,
                                 eos_RecordType1 *altEosData, EOS_REAL *p,
                                 EOS_REAL *e, EOS_REAL *a, EOS_REAL *s) {
  int i, j, ccLoaded;
  EOS_INTEGER ierr = EOS_OK;
  ...
  for (i = 0; i < me->NR; i++) {
    for (j = 0; j < me->NT; j++) {
      ...
      if (altEosData->T[i] <= ZERO)
        ...
        s[i + j * me->NR] =
            -(a[i + j * me->NR] - e[i + j * me->NR]) / altEosData->T[j];
      ...
      if (altEosData->T[j] > tmelt) {
        ...
        if (phif3 > (EOS_REAL)1.e-18) {
          ...
          p[i + j * me->NR] = UNIVERSAL_GAS_CONST / me->avgAtomicWgt *
                              altEosData->T[j] * altEosData->R[i] *
                              (ONE + gamf * phif);
          e[i + j * me->NR] = UNIVERSAL_GAS_CONST / me->avgAtomicWgt *
                              altEosData->T[j] * ((EOS_REAL)1.5) * (ONE + phif);

          /*! - Entropy and Helmholtz free energy equations taken from T-1's
           * opensesame (was Grizzly). */
          tcowan = (EOS_REAL)0.42 / ((EOS_REAL)22.0 + me->avgAtomicNumber);
          entropy_factor =
              SEVEN -
              (THREE * phif +
               ((EOS_REAL)1.5) * log(altEosData->T[j] / EV_TO_KELVIN *
                                     ((EOS_REAL)0.02) / (tcowan * tcowan))) -
              log(xi);
          s[i + j * me->NR] = entropy_factor * UNIVERSAL_GAS_CONST /
                              me->avgAtomicWgt; /* MJ/kg/K */
          a[i + j * me->NR] =
              e[i + j * me->NR] - altEosData->T[j] * s[i + j * me->NR];
        }
      } else {
        // compute debye-solid eos.
        gams = beta + (TWO)*rxi;
        gdens = gams * altEosData->R[i];
        tdebye = EV_TO_KELVIN * (EOS_REAL)1.68 * rxi2 *
                 (EOS_REAL)pow(xi, (TWO + beta)) /
        ...
        if (altEosData->T[j] > (tdebye * third)) {
          ...
          phis2 = phis * phis;
          e[i + j * me->NR] =
              THREE * UNIVERSAL_GAS_CONST / me->avgAtomicWgt *
              altEosData->T[j] *
              (ONE + phis2 * ((EOS_REAL)0.05 - phis2 * ONE / (EOS_REAL)1680));
          p[i + j * me->NR] = e[i + j * me->NR] * gdens;

          /* Entropy equation taken from T-1's opensesmame (was Grizzly): */
          entropy_factor =
              FOUR + THREE * (-log(phis) + phis2 * ((EOS_REAL)0.025 -
                                                    phis2 / (EOS_REAL)2240));
          s[i + j * me->NR] = entropy_factor * UNIVERSAL_GAS_CONST /
                              me->avgAtomicWgt; /* MJ/kg/K */
          a[i + j * me->NR] =
          ...
          eterm = ((EOS_REAL)pow(pi, (EOS_REAL)3) / FIVE * THREE) * rphis *
                      rphis * rphis -
                  (NINE + (EOS_REAL)27 * rphis *
                              (ONE + rphis * (TWO + rphis + rphis))) *
                      xphis;
          e[i + j * me->NR] =
              ((EOS_REAL)1.125) * UNIVERSAL_GAS_CONST / me->avgAtomicWgt *
                  tdebye +
...
/***********************************************************************/
/*!
 * \brief Compute the number-proportional pressure, internal energy, entropy,
 ...
 *
 ***********************************************************************/
EOS_INTEGER eos_computeNumPropData(eos_RecordType1 *me,
                                   eos_RecordType1 *altEosData, EOS_REAL *p,
                                   EOS_REAL *e, EOS_REAL *a, EOS_REAL *s) {
  int i, j, ccLoaded;
  ...
  for (i = 0; i < me->NR; i++) {
    ...
    for (j = 0; j < me->NT; j++) {
      ...
      z0 = (EOS_REAL)MAX(ONE, (prtot[i + j * me->NR] - pcold * mult) /
                                  (UNIVERSAL_GAS_CONST / me->avgAtomicWgt *
                                   altEosData->T[j] * altEosData->R[i]));
      ...
      z1 = cfermi * altEosData->T[j] * altEosData->T[j] *
           pow(altEosData->T[j], ONE / TWO) /
           (UNIVERSAL_GAS_CONST / me->avgAtomicWgt * altEosData->T[j] *
            altEosData->R[i]);
      it = 0;
      ...
      // calculate ion data
      p[i + j * me->NR] = (prtot[i + j * me->NR] - pcold * mult) / (ONE + zf);
      e[i + j * me->NR] = (entot[i + j * me->NR] - ecold * mult) / (ONE + zf);
  ...
  if (altEosData->eosData.numSubtablesLoaded <= 2) {
    ...
    /* calculate entropy */
    ierr = eos_Entropy(me, me->NR, me->NT, e, EOS_NullPtr, altEosData->T,
                       altEosData->R, s);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(ierr) != EOS_OK) {
      if (ierr == -2 || ierr == -3) {
        /* ignore the error, just set the dataSize and numTables loaded */
        me->eosData.dataSize =
            2 + me->NR + me->NT + 2 * me->NR * me->NT; /* only TWO subtables */
        me->eosData.numSubtablesLoaded = 2;
        ierr = EOS_OK;
      } else
        return ierr;
    ...
    for (i = 0; i < me->NR; i++) {
      acold = antot[i];
      for (j = 0; j < me->NT; j++) {
        a[i + j * me->NR] = (antot[i + j * me->NR] - mult * acold) /
    ...
    /* calculate entropy */
    ierr =
        eos_Entropy(me, me->NR, me->NT, e, a, altEosData->T, altEosData->R, s);
    if (eos_GetStandardErrorCodeFromCustomErrorCode(ierr) != EOS_OK) {
      if (ierr == -2 || ierr == -3) {
        /* ignore the error, just set the dataSize and numTables loaded */
        me->eosData.dataSize =
  ...
  /* Using total 301 table data, calculate appropriate data for requested table:
   * 303, 304 or 305 */
  for (i = 0; i < me->NR; i++) {
    for (j = 0; j < me->NT; j++) {
...
/***********************************************************************/
/*!
 * \brief Determines ion pressure, internal energy, and free energy using one of
 *the following:
 *    - the cowan-nuclear model (
 *      <a href="http://lib-www.lanl.gov/cgi-bin/getfile?00309855.pdf"
 *target=_blank>la-7313-ms</a>, and T-1's Grizzly code)
 *    - the ideal gas model
 *    - the Number-proportional model
 *
 * \param[out]   read_data - EOS_REAL : locally allocated array containing
 *                                         density (Mg/m3), temperature
 *(kelvin), ion pressure (Gj/m3), internal energy/mass (Gj/Mg), and free
 *energy/mass (Gj/Mg) \param[out]   **errMsg  - EOS_CHAR** : custom error
 *message \param[in]    *me       - eos_RecordType1 : pointer to the instance of
 *type eos_RecordType1 \param[in]    imodel    - EOS_INTEGER : desired model
 *option flag
 *
...
EOS_INTEGER eos_AnalyticalEOS(eos_RecordType1 *me, const EOS_INTEGER imodel,
  ...
  eos_CreateTablesEosDataMap(&gEosDataMap, 1, &datatype,
                             &me->eosData.materialID, &alt_handle, EOS_TRUE, 0,
                             EOS_FALSE, -1, &ierr);
  if (ierr)
    return (ierr);
  altEosData =
  ...
  // Cannot assume NR & NT are same for 301, 303, 304, 305 ... ; thus,
  // reallocation of 'me' may be needed.
  if (me->NR != altEosData->NR || me->NT != altEosData->NT)
    eos_SetSizeRecordType1(me, altEosData->NR, altEosData->NT,
                           me->eosData.tableNum);
  me->NR = altEosData->NR;
  me->NT = altEosData->NT;
  // store the total size of the data to be stored for all subtables:
  ...
  if (altEosData->T[0] > 0.0 && altEosData->eosData.numSubtablesLoaded < 3) {
    ...
    eos_SetCustomMsg_str(
        errMsg, "EOS_SPLIT_FAILED: The data splitting algorithm failed because "
                "T[0]>0 and no free energy table exists.");
    return (ierr);
  }
  ...
  *read_data =
  ...
  if (!(me->R && me->T && me->table1 && me->table2 && me->table3 && me->table4))
    eos_SetSizeRecordType1(me, me->NR, me->NT, me->eosData.tableNum);
  ...
  case EOS_SPLIT_NUM_PROP:
    ...
    /*! case EOS_SPLIT_NUM_PROP: Number Proportional data splitting model
     * details */
...
/***********************************************************************/
/*!
 * \brief adds points in between grid points of class eos_RecordType1 if the
 *option is set
 *
 * \param[out]   *err - EOS_INTEGER : error code
 * \param[in]    *ptr - void : data object pointer;
...
void eos_ExpandGridRecordType1(void *ptr, EOS_INTEGER *err) {
  ...
  *err = EOS_OK;
  ...
  for (i = 0; i < MAX_TABLES_RECORDTYPE1; i++) {
    ...
    _eos_GetDataRecordType1(me, &R, &T, &F, &coldCurve, i + 1);
    eos_ExpandGridInterpolate(nAdd, oldNR, oldNT, oldR, oldT, oldF[i], R, T, F,
                              err);

    /* also interpolate new coldCurve */
    if ((me->eosData.tableNum == 301 || me->eosData.tableNum == 303) &&
        coldCurve) {
      xyBounds = (EOS_INTEGER *)malloc(sizeof(EOS_INTEGER) *
                                       ((oldNR - 1) * nAdd + oldNR));
...
void eos_GetTableInfoRecordType1(void *ptr, EOS_INTEGER th,
  ...
  *err = EOS_OK;
  ...
  /* if either EOS_ALLOW_ALL_INFO_ITEMS option or me->isInvertedAtSetup is set,
   * then lift data type restrictions */
  allow_all_info_items = me->isInvertedAtSetup ||
                         gEosDataMap
                             .generalOptions[EOS_GENERAL_OPTION_FLAG_TO_INDEX(
                                 EOS_ALLOW_ALL_INFO_ITEMS)][th]
                             .bval;
  ...
  if (EOS_ALLOW_CAT0_ONLY_INFO_ITEM(infoItems[0]) || allow_all_info_items) {
    ...
    if (EOS_CATEGORY(eos_GetDataTypeFromTableHandle(th, err)) != 0 &&
      ...
      *err =
          eos_SetCustomErrorMsg(th, *err,
                                "Invalid info flag. This info flag is only "
                                "valid for simple, non-inverted data types.");
      return;
    } else {
      /* process x-conversion option */
      xconv = eos_getRealOptionFromTableHandle(th, EOS_X_CONVERT, err);
  ...
  case EOS_EL401:
    if (me->rt2_handle >= 0) {
      ...
      *err = EOS_INVALID_TABLE_HANDLE;
      *err = eos_SetCustomErrorMsg(th, *err,
                                   "Invalid table handle. 401 table data not "
                                   "available to supplied table handle.");
      return;
    }
    break;
  default:
    ...
    if (numInfoItems < me->NR) {
      ...
      *err =
          eos_SetCustomErrorMsg(th, *err,
                                ...
                                "allocation assumed because numInfoItems<NR.");
      return;
    }
  ...
  if (infoItems[0] == EOS_T_Array) {
    if (numInfoItems < me->NT) {
      ...
      *err =
          eos_SetCustomErrorMsg(th, *err,
                                "Operation failed. Insufficient memory "
                                "allocation assumed because numInfoItems<NT.");
      return;
    }
  ...
  if (infoItems[0] == EOS_X_Species_Data ||
    ...
    EOS_INTEGER nXYPairs =
        gEosInterpolation.interpolationDataList[th]->nXYPairs;
    if (infoItems[0] == EOS_X_Species_Data) {
    ...
    if (!p) {
      *err = EOS_WARNING;
      *err = eos_SetCustomErrorMsg(th, *err,
                                   "Operation failed. The requested data (%s) "
                                   "is unavailable for table handle %d.",
                                   s, th);
      return;
    }
    if (numInfoItems < nXYPairs) {
      ...
      *err = eos_SetCustomErrorMsg(
          th, *err,
          "Operation failed. Insufficient memory allocation assumed because "
          "numInfoItems<nTables.");
      return;
    }
  ...
  for (i = 0; i < numInfoItems; i++) {
    ...
    case EOS_Log_Val:
      infoVals[i] = (EOS_REAL)EOS_TYPE_TO_LOG_AXES(
          eos_GetDataTypeFromTableHandle(th, err));
      break;
    ...
    case EOS_nXYPairs:
      infoVals[i] =
          (EOS_REAL)gEosInterpolation.interpolationDataList[th]->nXYPairs;
      break;
    ...
    case EOS_dFy_Species_Data:
      ...
      *err = EOS_INVALID_INFO_FLAG;
      *err = eos_SetCustomErrorMsg(th, *err,
                                   "Invalid info flag because %s may only be "
                                   "used without any other info flags.",
                                   s);
      break;
 ...
 *
 * Input Value:
 * void *ptr           - this pointer (pointer to the instance of type
 *eos_RecordType1 EOS_CHAR *infoItem  - flag specifying what meta data item to
 *fetch
 *
 ************************************************************************/
 ...
 * \param[in]    NR        - EOS_INTEGER : number of density dependent values in
 *arrays \param[in]    NT        - EOS_INTEGER : number of temperature dependent
 *values in arrays \param[in]    U[NR*NT]  - EOS_REAL : array of internal energy
 *data (Gj/Mg); it may include the cold curve component \param[in]    F[NR*NT]
 *- EOS_REAL : array of free energy data (Gj/Mg) \param[in]    T[NT]     -
 *EOS_REAL : array of temperature data (K)
 *
 * \return eos_Entropy - EOS_INTEGER : error code
 *                            EOS_NOT_ALLOCATED indicates array(s) not allocated
 *for data -2                indicates T=zero isotherm not in data -3 indicates
 *EOS_PT_SMOOTHING option is set
 *
 ***********************************************************************/
EOS_INTEGER eos_Entropy(eos_RecordType1 *me, EOS_INTEGER NR, EOS_INTEGER NT,
                        EOS_REAL *U, EOS_REAL *F, EOS_REAL *T, EOS_REAL *R,
                        EOS_REAL *S) {
  int i, j;
  EOS_INTEGER ierr = EOS_OK;
  ...
  if (!F && T[0] > ZERO) {      // T=zero isotherm and F not in data
     ...
     *        if free energy is unavailable; otherwise the current results of
     * this function are incorrect when no cold curve data is tabulated. */
    return -2;
  }
  ...
  if (F) {
    /*! When free energy data is available, use \f$ S=\frac{(U-A)}{T} \f$ to
     * calculate entropy. Calculate entropy using the definition of free energy,
     * and force \f$ S=0 \f$ at \f$ T=0 \f$.
     */
    for (i = 0; i < NR; i++) {
      for (j = 0; j < NT; j++) {
        S[i + j * NR] =
            (T[j] <= ZERO) ? ZERO : (U[i + j * NR] - F[i + j * NR]) / T[j];
      }
    }
  } else {
    /*! When free energy data is not available, use \f$ S = \int_{0}^{T}
     * \frac{1}{T} \frac{dU}{dT}dT \f$ to calculate entropy, where U is the
     * total internal energy. Calculate entropy using the definition of free
     * energy, and force \f$ S=0 \f$ at \f$ T=0 \f$.
     *
    ...
    for (i = 0; i < NR; i++) {
      ...
      /*! Calculate values of integrand data, \f$ \frac{U}{{T}^{2}} \f$, and
       * force to zero when \f$ T \leq 0 \f$.
       */
      for (j = 0; j < NT; j++) {
        integrandValues[j] =
      ...
      for (j = 1; j < NT; j++) {
        if (T[j] <= ZERO) {
          ...
          ierr = eos_TrapezoidIntegrate(
              j - 1, j, T[j], NT, integrandValues, T, 99, &integralValue,
              (!(i == (NR - 1) && j == (NT - 1))) ? EOS_TRUE : EOS_FALSE);
 ...
 *
 ***********************************************************************/
void eos_GetLoadedBulkDataRecordType1(void *ptr, EOS_REAL *zbar, EOS_REAL *abar,
...
void eos_CleanUpColdCurveRecordType1(void *ptr, EOS_INTEGER *err) {
  ...
  EOS_REAL temp, *newR = NULL, *tableRow1 = NULL, *tableRow2 = NULL,
                 *tableRow3 = NULL, *tableRow4 = NULL;
  me = (eos_RecordType1 *)ptr;
  *err = EOS_OK;
  ...
  if (tableRow4)
    EOS_FREE(tableRow4);
  me->eosData.coldCurveIsLoaded =
      EOS_FALSE;            /* indicates that cold curves arrays are null! */
 ...
 *
 * \param[in]    dataType     - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[out]   *isSmooth    - EOS_INTEGER : is data  of *ptr
 *eos_RecordType1 object smooth according to EOS_SMOOTH option? \param[out]
 **isPtSmooth  - EOS_INTEGER : is data  of *ptr eos_RecordType1 object smooth
 *                                            according to EOS_PT_SMOOTHING
 *option? \param[in]    *ptr         - void : data object pointer; internally
 *recast to eos_RecordType1*
 *
 * \return none
 *
 ***********************************************************************/
 ...
 *
 * \param[in]    dataType     - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[out]   *inX         - EOS_INTEGER : is data  of *ptr
 *eos_RecordType1 object monotonic with respect to first independent variable?
 * \param[out]   *inY         - EOS_INTEGER : is data  of *ptr eos_RecordType1
 *object monotonic with respect to second independent variable? \param[in] *ptr
 *- void : data object pointer; internally recast to eos_RecordType1*
 *
 * \return none
 *
 ***********************************************************************/
...
/***********************************************************************/
/*!
 * \brief Set the flags so that appropriate data will be made monotonic upon
 *loading
 *
 * \param[in]     *ptr    - void : data object pointer;
 *                                 internally recast to eos_RecordType1*
 * \param[in]    dataType - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[in]    inX      - EOS_INTEGER : is data  of *ptr eos_RecordType1
 *object monotonic with respect to first independent variable? \param[in]    inY
 *- EOS_INTEGER : is data  of *ptr eos_RecordType1 object monotonic with respect
 *to second independent variable?
 *
 * \return none
 *
 ***********************************************************************/
 ...
 *  object
 *
 * \param[out]   *compatible  - EOS_BOOLEAN : are the monotonic options
 *compatible? \param[in]    *ptr         - void : data object pointer;
 *                                     internally recast to eos_RecordType1*
 * \param[in]    dataType     - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[in]    inX          - EOS_INTEGER : is data  of *ptr
 *eos_RecordType1 object monotonic with respect to first independent variable?
 *                                            (ignore if negative)
 * \param[in]    inY          - EOS_INTEGER : is data  of *ptr eos_RecordType1
 *object monotonic with respect to second independent variable? (ignore if
 *negative)
 *
 * \return none
 *
 ***********************************************************************/
void eos_AreMonotonicRequirementsCompatibleRecordType1(
    void *ptr, EOS_INTEGER dataType, EOS_INTEGER inX, EOS_INTEGER inY,
    EOS_BOOLEAN *compatible) {
  EOS_INTEGER subTableNum = 1, subTableNumStart, subTableNumEnd;
  eos_RecordType1 *me;
  EOS_INTEGER *shouldBeMonotonicX = 0, *shouldBeMonotonicY = 0,
              *shouldBePtSmooth = 0;
  ...
  *compatible = EOS_TRUE;
  ...
  for (subTableNum = subTableNumStart; subTableNum <= subTableNumEnd;
    ...
    if (inX >= 0 && *shouldBeMonotonicX != inX)
      *compatible = EOS_FALSE;
    if (inY >= 0 && *shouldBeMonotonicY != inY)
      *compatible = EOS_FALSE;
    if (inX != 0 || (inY != 0 && *shouldBePtSmooth > 0))
      *compatible = EOS_FALSE;
...
/***********************************************************************/
/*!
 * \brief Set the flags so that appropriate data will be made smooth upon
 *loading
 *
 * \param[in]    *ptr      - void : data object pointer;
 *                                  internally recast to eos_RecordType1*
 * \param[in]    dataType  - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[in]    pt_smooth - EOS_INTEGER : is data  of *ptr
 *eos_RecordType1 object smooth according to EOS_PT_SMOOTHING option? \param[in]
 *smooth    - EOS_INTEGER : is data  of *ptr eos_RecordType1 object smooth
...
void eos_SetSmoothingRecordType1(void *ptr, EOS_INTEGER dataType,
  ...
  me->shouldBePtSmooth1 = me->shouldBePtSmooth2 = me->shouldBePtSmooth3 =
      me->shouldBePtSmooth4 = (pt_smooth != 0);
...
/***********************************************************************/
/*!
 * \brief Find out if the requested type, smoothing combo can share our table
 *object
 *
 * \param[out]   *compatible  - EOS_BOOLEAN : are the monotonic options
 *compatible? \param[in]    *ptr         - void : data object pointer;
 *                                     internally recast to eos_RecordType1*
 * \param[in]    dataType     - EOS_INTEGER : data type of *ptr eos_RecordType1
 *object \param[in]    makeSmooth   - EOS_INTEGER : is data  of *ptr
 *eos_RecordType1 object smooth with respect to EOS_SMOOTH option? \param[in]
 *makePtSmooth - EOS_INTEGER : is data  of *ptr eos_RecordType1 object smooth
 *                                            with respect to EOS_PT_SMOOTHING
 *option?
 *
 * \return none
 *
 ***********************************************************************/
void eos_AreSmoothingRequirementsCompatibleRecordType1(
    void *ptr, EOS_INTEGER dataType, EOS_INTEGER makeSmooth,
 ...
 *
 * \param[out]   *fVals     - EOS_REAL : nXYPairs of specific volume or specific
 *internal energy \param[out]   *dFx       - EOS_REAL : nXYPairs of partial
 *derivatives w.r.t. pressure \param[out]   **dFy      - EOS_REAL : nXYPairs of
 *partial derivatives w.r.t. temperature \param[out]   *errorCode - EOS_INTEGER
 *: error code \param[out]   *xyBounds  - EOS_INTEGER : interpolation errors per
 *xy-pair \param[in]    *me        - eos_RecordType1 : data object pointer
 * \param[in]    dataType   - EOS_INTEGER : data type
 * \param[in]    nXYPairs   - EOS_INTEGER : number of temperature-pressure datum
 *pairs \param[in]    *pres      - EOS_REAL : nXYPairs of pressure values
...
void _eos_SesameInvTRecordType1(eos_RecordType1 *me, EOS_INTEGER dataType,
                                EOS_INTEGER nXYPairs, EOS_REAL *pres,
                                EOS_REAL *temp, EOS_REAL *fVals, EOS_REAL *dFx,
                                EOS_REAL *dFy, EOS_INTEGER *xyBounds,
                                EOS_INTEGER *errorCode, EOS_CHAR **errMsg) {
  EOS_INTEGER irlo, irhi, ir, j;
  EOS_REAL sr, sv, vol;
  EOS_INTEGER isotherm_nr, err;
  EOS_REAL *isotherm_p = NULL, *isotherm_e = NULL, *isotherm_r = NULL,
           *isotherm_v = NULL;
    ...
    if (dataType == EOS_V_PtT) {
      ...
      */
      if (pres[j] < isotherm_p[0]) {
        ...
        /* _eos_sesame_isotherm_buildRecordType1 can return ZERO in isotherm_p
           and isotherm_r arrays,
           and pres array can contain ZERO -- appropriate checks must be made */

        /* isotherm_p may contain zero which is minimum possible -- see
         * _eos_sesame_isotherm_buildRecordType1 */
        /* isotherm_r may contain zero which is minimum possible -- see SESAME
         * data */
        ...
        */
        ...
        if (fabs(fVals[j]) <= TINY_D || fabs(isotherm_p[0]) <= TINY_D) {
          ...
          err =
              EOS_INTERP_EXTRAPOLATED; /* store err until j-loop is finished */
        }
        fVals[j] = ONE / FLOOR(fVals[j]);
        /* 	dFx[j] = ????;  THIS IS CURRENTLY UNDEFINED AS DOCUMENTED IN
         * THE USER MANUAL -- DAP */
        /* 	dFy[j] = ????;  THIS IS CURRENTLY UNDEFINED AS DOCUMENTED IN
         * THE USER MANUAL -- DAP */
        ...
        /* pres may contain negative, zero and/or positive values -- input */
        /* ORIGINAL CODE:
           fVals[j] = ONE / (isotherm_r[isotherm_nr] * (pres[j] /
           isotherm_p[isotherm_nr]));
        */
        fVals[j] = isotherm_r[isotherm_nr] *
                   (pres[j] / FLOOR(isotherm_p[isotherm_nr]));
        if (fabs(fVals[j]) < TINY_D ||
            fabs(isotherm_p[isotherm_nr]) <= TINY_D) {
          xyBounds[j] = EOS_CANT_INVERT_DATA;
        ...
        sr = (pres[j] - isotherm_p[irlo]) /
             FLOOR(isotherm_p[irhi] - isotherm_p[irlo]);
        fVals[j] = ONE / FLOOR(isotherm_r[irlo] +
                               sr * (isotherm_r[irhi] - isotherm_r[irlo]));

        if (fabs(isotherm_r[irlo] +
                 sr * (isotherm_r[irhi] - isotherm_r[irlo])) <= TINY_D) {
          xyBounds[j] = EOS_CANT_INVERT_DATA;
          err =
              EOS_INTERP_EXTRAPOLATED; /* store err until j-loop is finished */
        }
      ...
      /*
        Calculate the specific internal energy values for all pres[j] and
        temp[j] values
      */
      if (pres[j] < isotherm_p[0]) {
        ...
        sr = (pres[j] - isotherm_p[irlo]) /
             (isotherm_p[irhi] - isotherm_p[irlo]);
        vol = ONE /
              (isotherm_r[irlo] + sr * (isotherm_r[irhi] - isotherm_r[irlo]));
 ...
 *  given temperature.
 *
 * \param[out]   *nr         - EOS_INTEGER : number of isotherm data values
 *returned \param[out]   *isotherm_p - EOS_REAL : pressure values \param[out]
 **isotherm_e - EOS_REAL : specific internal energy isotherm \param[out]
 **isotherm_r - EOS_REAL : density isotherm \param[out]   *isotherm_v - EOS_REAL
 *: specific volume isotherm \param[out]   *errorCode  - EOS_INTEGER : error
 *code \param[in]    *me         - eos_RecordType1 : data object pointer
 * \param[in]    temp        - EOS_REAL : temperature value
 *
 * \return none
 *
 ***********************************************************************/
void _eos_sesame_isotherm_buildRecordType1(
    eos_RecordType1 *me, EOS_REAL temp, EOS_INTEGER *isotherm_nr,
  ...
  EOS_REAL *vapor_t = NULL, *vapor_p = NULL, *vapor_rl = NULL, *vapor_rg = NULL,
           *vapor_vl = NULL, *vapor_vg = NULL, *vapor_el = NULL,
           *vapor_eg = NULL;
  ...
  *isotherm_v = (EOS_REAL *)malloc((isotherm_dim) * sizeof(EOS_REAL));
  ...
  if (temp < me->T[0]) {
    which = -1;
    st = temp / me->T[0]; /* previous temp check ensured temp >= zero; thus,
                             me->T[0] > zero here */
    itlo = 1;
    ithi = 2;
  ...
  for (ir = 0; ir < me->NR; ir++) {
    ...
    default:
      p = me->table1[itlo][ir] +
          st * (me->table1[ithi][ir] - me->table1[itlo][ir]);
      e = me->table2[itlo][ir] +
          st * (me->table2[ithi][ir] - me->table2[itlo][ir]);
      break;
    }
  ...
  if (temp >= tcrit)
    ...
    /* if (irlo.le.0)    call global_error('SESAME_ISOTHERM_BUILD: irhi.gt.0
     * .and. irlo.le.0') */
    if (irlo < 0) {
      *errorCode = EOS_FAILED;
      eos_SetCustomMsg_str(
    ...
    /* if (irlo.ge.irhi) call global_error('SESAME_ISOTHERM_BUILD:
     * irlo.ge.irhi') */
    if (irlo >= irhi) {
      *errorCode = EOS_FAILED;
      eos_SetCustomMsg_str(
          errMsg, "_eos_sesame_isotherm_buildRecordType1: irlo >= irhi");
      return;
    }
  ...
  case 0:
    ...
    ighi = iglo + 1;
    s = MAX(ZERO, MIN(ONE, (plg - scr_p[iglo]) / (scr_p[ighi] - scr_p[iglo])));
    ...
    ilhi = illo + 1;
    s = MAX(ZERO, MIN(ONE, (plg - scr_p[illo]) / (scr_p[ilhi] - scr_p[illo])));
  ...
  default:
    ...
    *errorCode = EOS_FAILED;
    eos_SetCustomMsg_str(
        errMsg, "_eos_sesame_isotherm_buildRecordType1: unknown use_flag");
    break;
  }
  ...
  _eos_DEBUG_PRINT("%2s %12s %12s %12s %12s\n", "ir", "temp", "isotherm_p",
                   "isotherm_e", "isotherm_r");
  for (ir = 0; ir <= *isotherm_nr; ir++)
    _eos_DEBUG_PRINT("%2i %12.4e %12.4e %12.4e %12.4e %12.4e\n", ir, temp,
                     (*isotherm_p)[ir], (*isotherm_e)[ir], (*isotherm_r)[ir]);
  ...
  for (ir = 1; ir <= *isotherm_nr; ir++) {
    ...
    if ((*isotherm_p)[ir] < (*isotherm_p)[ir - 1]) {
      ...
      /*               endif */
      _eos_DEBUG_PRINT("_eos_sesame_isotherm_buildRecordType1: ir, t, p1, p2 = "
                       "%6i%12.4e%20.12e%20.12e\n",
                       ir, temp, (*isotherm_p)[ir - 1], (*isotherm_p)[ir]);
    }
    ...
    if ((*isotherm_r)[ir] < (*isotherm_r)[ir - 1]) {
      ...
      /*               endif */
      _eos_DEBUG_PRINT("_eos_sesame_isotherm_buildRecordType1: ir, t, r1, r2 = "
                       "%6i%12.4e%20.12e%20.12e\n",
                       ir, temp, (*isotherm_r)[ir - 1], (*isotherm_r)[ir]);
    }
  ...
  if (bad) {
    for (ir = 0; ir <= scr_num; ir++) {
      ...
      _eos_DEBUG_PRINT(
          "_eos_sesame_isotherm_buildRecordType1: $$$ ir, scr_r, scr_p, scr_e "
          "= %6i%12.4e%12.4e%12.4e%12.4e%12.4e%12.4e\n",
          ir, temp, scr_r[ir], scr_p[ir], scr_e[ir]);
    } /* ir */
    for (ir = 0; ir <= *isotherm_nr; ir++) {
      /*               write(*,"(a,i6,1p,6e12.4)")'$$$ ir, isotherm_r,
       * isotherm_p, isotherm_e = ', & */
      /*                    ir, isotherm_r(ir), isotherm_p(ir), isotherm_e(ir)
       */
      _eos_DEBUG_PRINT(
          "_eos_sesame_isotherm_buildRecordType1: $$$ ir, isotherm_r, "
          "isotherm_p, isotherm_e = %6i%12.4e%12.4e%12.4e%12.4e%12.4e%12.4e\n",
