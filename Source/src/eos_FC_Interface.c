                                  ...
                                  EOS_INTEGER *errorCode)
{
  ...
  *errorCode = EOS_OK;
  ...
  if (! gEosDataMap.initialized) { //firstTime) {
    ...
    firstTime = EOS_FALSE;
    gEosDataMap.initialized = EOS_TRUE; /* once this is initialized, it remains so until either eos_DestroyEosDataMap is called via eos_DestroyAll or execution stops */
                                   ...
                                   EOS_INTEGER *errorCode)
{
  ...
  for (i = 0; i < *nTables; i++) {
    /* Initialize errorCode */
    *errorCode = EOS_OK;
    if (tableHandles[i] < 0) /* negative tableHandle will cause fatal errors */
    {
      *errorCode = EOS_INVALID_TABLE_HANDLE;
      continue;
    }
                                 ...
                                 EOS_CHAR *infoStr, EOS_INTEGER *errorCode)
{
  ...
  *errorCode = EOS_OK;
  ...
  if ( *infoItem > EOS_NullTable
       && *infoItem <= _eos_TableListReverseMap_size
                                      ...
                                      EOS_CHAR *infoStr, EOS_INTEGER *errorCode)
{
  ...
  *errorCode = EOS_OK;
  ...
  {
    *errorCode = EOS_INVALID_TABLE_HANDLE;
    goto eos_GetTableMetaData_CLEANUP;
  }
                                   ...
                                   EOS_INTEGER *errorCode)
{
  ...
  *errorCode = EOS_OK;
  ...
  {
    *errorCode = EOS_INVALID_TABLE_HANDLE;
    goto eos_GetTableCmnts_CLEANUP;
  }
                                     ...
                                     long int fileName_len)
{
  ...
  *errorCode = EOS_OK;
  ...
  //_len_ = MIN(strlen(fileName),MAX(fileName_len-1,0));
  if      (fileName_len <= 0       ) _len_ = MIN(strlen(fileName), PATH_MAX) - 1;
  else if (fileName_len <  PATH_MAX) _len_ = MIN(strlen(fileName), fileName_len) - 1;
  else                               _len_ = fileName_len - 1;
  for (i = MIN(_len_,PATH_MAX-1); i >= 0; i--) {
    if (fileName[i] == ' ' || fileName[i] == '\0') continue;
    else                                           break;
  }
