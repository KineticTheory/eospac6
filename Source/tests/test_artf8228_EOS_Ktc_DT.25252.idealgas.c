...
/*! \file
 *  \ingroup tests
 *  \brief Check that the correct tables are being loaded for EOS_Ktc_DT
 *         and material 25252 with a forced EOS_SPLIT_IDEAL_GAS option.
 *
 * \note
 * MATIDS TO TEST: 25252
 */
...
int main(){
  ...
  tableOption = EOS_SPLIT_IDEAL_GAS;
  eos_SetOption(&table_handles[0], &tableOption, EOS_NullPtr, &error);
  /*check for errors*/
  if(error == EOS_OK)
    printf("eos_SetOption ... EOS_OK (EOS_SPLIT_IDEAL_GAS)\n");
  else {
    eos_GetErrorMessage (&errorCode, errorMessage);
    printf ("eos_SetOption ERROR %i: %s (EOS_SPLIT_IDEAL_GAS)\n", errorCode, errorMessage);
  }
  print_nr_and_nt(table_handles[0]);
