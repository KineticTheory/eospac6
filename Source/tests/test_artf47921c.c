...
int main ()
{
  ...
  modelTests[1] = make_test_entry(3720, 33720, EOS_Ut_DPt, EOS_Pt_DT, EOS_INVERT_AT_SETUP );
  ...
  { /* redefine modelTests[itest]->matID if required */
    ...
    if (myenvar != NULL) {      /* parse the whitespace- and/or comma-delimited list of values */
      ...
      for (token = strtok_r(myenvar, ", 	", &state); token != NULL; token = strtok_r(NULL, ", 	", &state)) {
        ...
        modelTests[nModelTests-1] = make_test_entry(3720, 33720, EOS_Ut_DPt, EOS_Pt_DT, EOS_INVERT_AT_SETUP+EOS_INSERT_DATA+i );
      }
    }
    else {
      nModelTests += 3;
      modelTests = (modelTest_s**) safe_realloc(modelTests, nModelTests, sizeof(modelTest_s*));
      modelTests[2] = make_test_entry(3720, 33720, EOS_Ut_DPt, EOS_Pt_DT, EOS_INVERT_AT_SETUP+EOS_INSERT_DATA+1 );
      modelTests[3] = make_test_entry(3720, 33720, EOS_Ut_DPt, EOS_Pt_DT, EOS_INVERT_AT_SETUP+EOS_INSERT_DATA+2 );
      modelTests[4] = make_test_entry(3720, 33720, EOS_Ut_DPt, EOS_Pt_DT, EOS_INVERT_AT_SETUP+EOS_INSERT_DATA+3 );
    }
  }
