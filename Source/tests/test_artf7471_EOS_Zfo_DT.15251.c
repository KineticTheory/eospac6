...
/*! \file
 *  \ingroup C tests
 *  \brief Check that the correct tables are being loaded for EOS_Zfo_DT
 *         and material 15251 with a forced EOS_SPLIT_NUM_PROP option.
 *
 * \note
 * MATIDS TO TEST: 15251
 */
...
int main(){
  ...
  tableOption = EOS_SPLIT_NUM_PROP;
  eos_SetOption(&table_handles[0], &tableOption, EOS_NullPtr, &error);
  /*check for errors*/
  if(error == EOS_OK)
    printf("eos_SetOption ... EOS_OK (EOS_SPLIT_NUM_PROP)\n");
  else {
    eos_GetErrorMessage (&errorCode, errorMessage);
    printf ("eos_SetOption ERROR %i: %s (EOS_SPLIT_NUM_PROP)\n", errorCode, errorMessage);
  }
  print_nr_and_nt(table_handles[0]);
