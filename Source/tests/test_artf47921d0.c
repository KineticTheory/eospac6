...
int main ()
{
  ...
  modelTests[0] = make_test_entry(3720, 33720, EOS_Pt_DUt, EOS_Ut_DT, 0 );
  modelTests[1] = make_test_entry(3720, 33720, EOS_Pt_DUt, EOS_Ut_DT, 0 );
  ...
  { /* redefine modelTests[itest]->matID if required */
    ...
    if (myenvar != NULL) {      /* parse the whitespace- and/or comma-delimited list of values */
      ...
      for (token = strtok_r(myenvar, ", 	", &state); token != NULL; token = strtok_r(NULL, ", 	", &state)) {
        ...
        modelTests[nModelTests-1] = make_test_entry(3720, 33720, EOS_Pt_DUt, EOS_Pt_DT, EOS_INSERT_DATA+i );
      }
    }
    else {
      nModelTests += 3;
      modelTests = (modelTest_s**) safe_realloc(modelTests, nModelTests, sizeof(modelTest_s*));
      modelTests[2] = make_test_entry(3720, 33720, EOS_Pt_DUt, EOS_Pt_DT, EOS_INSERT_DATA+1 );
      modelTests[3] = make_test_entry(3720, 33720, EOS_Pt_DUt, EOS_Pt_DT, EOS_INSERT_DATA+2 );
      modelTests[4] = make_test_entry(3720, 33720, EOS_Pt_DUt, EOS_Pt_DT, EOS_INSERT_DATA+3 );
    }
  }
  ...
  for (itest = 0; itest < nModelTests; itest++) {
    ...
    printf ("#\n# *********************************************************************\n");
    {
      EOS_INTEGER num = (EOS_INTEGER)(modelTests[itest]->modelFlag - EOS_INSERT_DATA);
      EOS_CHAR str[100];
      sprintf(str, " with EOS_INSERT_DATA=%d option", num);
      printf ("# *** TEST CASE %i: %s for material %i%s ***\n# %s\n", itest + 1,
              get_tableType_str(modelTests[itest]->tableType), modelTests[itest]->matID,
              ((modelTests[itest]->modelFlag) ? str : ""),
    ...
    if (modelTests[itest]->modelFlag) {
      EOS_REAL num = (EOS_REAL)(modelTests[itest]->modelFlag - EOS_INSERT_DATA);
  ...
  { /* Dump interpolated data comparisons for gnuplot heat maps */
    FILE *fp = fopen("test_artf47921d.heatmap.dat", "w");
    if(fp != NULL) {

      for (itest = 1; itest < nModelTests; itest++) {

        EOS_INTEGER num = (EOS_REAL)(modelTests[itest]->modelFlag - EOS_INSERT_DATA);
