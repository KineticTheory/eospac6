...
int main ()
{
  ...
  modelTests[0] = make_test_entry(3720, 33720, EOS_D_PtT, EOS_Pt_DT, 0 );
  modelTests[1] = make_test_entry(3720, 33720, EOS_D_PtT, EOS_Pt_DT, 0 );
  ...
  { /* redefine modelTests[itest]->matID if required */
    ...
    if (myenvar != NULL) {      /* parse the whitespace- and/or comma-delimited list of values */
      ...
      for (token = strtok_r(myenvar, ", 	", &state); token != NULL; token = strtok_r(NULL, ", 	", &state)) {
        ...
        modelTests[nModelTests-1] = make_test_entry(3720, 33720, EOS_D_PtT, EOS_Pt_DT, EOS_INSERT_DATA+i );
      }
    }
    else {
      nModelTests += 3;
      modelTests = (modelTest_s**) safe_realloc(modelTests, nModelTests, sizeof(modelTest_s*));
      modelTests[2] = make_test_entry(3720, 33720, EOS_D_PtT, EOS_Pt_DT, EOS_INSERT_DATA+1 );
      modelTests[3] = make_test_entry(3720, 33720, EOS_D_PtT, EOS_Pt_DT, EOS_INSERT_DATA+2 );
      modelTests[4] = make_test_entry(3720, 33720, EOS_D_PtT, EOS_Pt_DT, EOS_INSERT_DATA+3 );
    }
  }
  ...
  { /* Dump interpolated data comparisons for gnuplot heat maps */
    FILE *fp = fopen("test_artf47921e.heatmap.dat", "w");
    if(fp != NULL) {
