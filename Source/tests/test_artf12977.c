...
int main ()
{
  ...
  for (i = 0; i < nTables; i++) {
    ...
    eos_GetPackedTablesSize (&nHandles0, tableHandle, &packedTablesSize[i], &errorCode);
    ...
    eos_GetPackedTables (&nHandles0, tableHandle, packedData[i], &errorCode);
    if (errorCode != EOS_OK) {
      eos_GetErrorMessage (&errorCode, errorMessage);
    ...
    eos_SetPackedTables (&nHandles0, &packedTablesSize[i], packedData[i], tableHandle,
			 &errorCode);
    if (errorCode != EOS_OK) {
      eos_GetErrorMessage (&errorCode, errorMessage);
