...
ses_boolean _go_to_index_record_ascii(ses_file_handle the_handle, ses_material_id the_mid, ses_table_id the_tid) {
...
#ifdef DEBUG_PRINT
    printf("_go_to_index_record_ascii: Invalid handle passed to _go_to_index_record =  %d\n", the_handle);
#endif
    return SES_FALSE;
  }
...
#ifdef DEBUG_PRINT
    printf("_go_to_index_record_ascii: ses file handle null in _go_to_index_record \n");
#endif
    return SES_FALSE;
  }
  ...
  if (pFILE == NULL) {
#ifdef DEBUG_PRINT
    printf("_go_to_index_record_ascii: pFILE is null in _go_to_index record\n");
#endif
    return SES_FALSE;
  }
  ...
  if (ptDIR == NULL) {
#ifdef DEBUG_PRINT
    printf("_go_to_index_record_ascii: Directory is null in _go_to_index_record \n");
#endif
    return SES_FALSE;
  }
  /***************************************************************************************/
  ...
  long maddress = _get_address_for_material_ascii(ptDIR, the_mid, pSFH);   
  ...
  if (fseek_return != 0) {
#ifdef DEBUG_PRINT
    printf("_go_to_index_record: fseek return in _go_to_index record_ascii error \n");
#endif
    return SES_FALSE;
  }
