...
long _get_standard_num_arrays(ses_table_id the_tid) {
  ...
  if (index < 0) {
#ifdef DEBUG_PRINT
    //printf("_get_standard_num_arrays: my_get_index failed in _get_standard_num_arrays\n");
#endif
...
ses_boolean my_read_tables(void) {
  ...
  the_string =  malloc(sizeof(char) * MAX_LENGTH_STRING);  //  create memory
  strcpy(the_string, "{ tid: 401, num_arrays : 8, num_independent : 1, description : \"Vaporization Table\", sizes : [ (1), (nt), (nt), (nt), (nt), (nt), (nt), (nt)]  , labels: [ \"nt (number temperatures)\",\"p - vapor pressure (GPa)\", \"t - temperature (K)\", \"rg - vapor density on coexistence line (Mg/m3)\", \"rl - density of liquid or solid on coexistence line (Mg/m3)\", \"eg - internal energy of vapor on coexistence line (MJ/kg)\", \"el - internal energy of liquid or solid on coexistence line (MJ/kg)\", \"ag - free energy of vapor on coexistence line (MJ/kg)\", \"al - free energy of liquid or solid on coexistence line (MJ/kg)\"]}");
  _the_tables[index] = _construct_standard_table((ses_string)the_string);
#ifdef DEBUG_MY_READ_TABLES
   _print_standard_table(_the_tables[index]);
#endif
