...
ses_error_flag ses_setup(ses_file_handle the_handle, ses_material_id the_mid, ses_table_id the_tid) {
    ...
    switch(open_flags) {
        ...
        case 'A':
            ...
            if ((nr <= 0) || (nt <= 0)) {
#ifdef DEBUG_PRINT
                printf("ses_setup (for appendx):  grid not defined (nr or nt is <= 0)\n");
#endif
...
#ifdef DEBUG_PRINT
    printf("_do_setup_for_read: _read_my_directory\n");
#endif
...
#ifdef DEBUG_PRINT
    printf("_do_setup_for_read: _read_current_index_record\n");
#endif
...
#ifdef DEBUG_PRINT
        printf("_construct_current_data_record: 16\n");
#endif
   }
